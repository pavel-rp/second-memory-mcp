...
- **Prompt engineering surface** – Eight MCP prompt templates (scaffolding, learning, retrieval, review, workflow guidance, chunk generation, chunk management, learning session orchestration) that give the connected AI client structured context for each phase of a session (`src/shared/prompts/prompt-pack.ts`).
...
The server also exposes a `learning_session` MCP prompt resource (discoverable via `prompts/list`) that provides session orchestration guidance — how to start, conduct, and complete a learning session using the tools above. Clients can invoke it with optional `sessionMode`, `timeAvailable`, and `subject` arguments.

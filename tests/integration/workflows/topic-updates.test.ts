...
describe('services/topic-updates', () => {
  ...
  describe('updateTopicMetadata', () => {
    it('updates title only', async () => {
      ...
      const result = await ctx.updateTopicMetadata('t1', { title: 'New Title' });
    ...
    it('updates subject only', async () => {
      await insertTopic('t1');
      const result = await ctx.updateTopicMetadata('t1', { subject: 'Physics' });
      expect(result.success).toBe(true);
    ...
    it('updates both title and subject', async () => {
      await insertTopic('t1');
      const result = await ctx.updateTopicMetadata('t1', {
        title: 'Updated',
        subject: 'Science',
      });
      expect(result.success).toBe(true);
    ...
    it('rejects empty title', async () => {
      await insertTopic('t1');
      const result = await ctx.updateTopicMetadata('t1', { title: '' });
    ...
    it('rejects empty subject', async () => {
      await insertTopic('t1');
      const result = await ctx.updateTopicMetadata('t1', { subject: '' });
      expect(result.success).toBe(false);
      expect(result.error?.type).toBe('validation');
    ...
    it('returns not_found for non-existent topic', async () => {
      const result = await ctx.updateTopicMetadata('nonexistent', { title: 'Foo' });
      expect(result.success).toBe(false);
  ...
  describe('updateTopicSummary', () => {
    it('updates summary and increments version', async () => {
      await insertTopic('t1');
      const result = await ctx.updateTopicSummary('t1', 'A great summary');
      expect(result.success).toBe(true);
    ...
    it('increments version on each update', async () => {
      await insertTopic('t1');
      await ctx.updateTopicSummary('t1', 'Version 2');
      const result = await ctx.updateTopicSummary('t1', 'Version 3');
      expect(result.success).toBe(true);
    ...
    it('rejects empty summary', async () => {
      await insertTopic('t1');
      const result = await ctx.updateTopicSummary('t1', '');
      expect(result.success).toBe(false);
      expect(result.error?.type).toBe('validation');
    ...
    it('rejects summary exceeding max size', async () => {
      await insertTopic('t1');
      const result = await ctx.updateTopicSummary(
        't1',
    ...
    it('returns not_found for non-existent topic', async () => {
      const result = await ctx.updateTopicSummary('nonexistent', 'summary');
      expect(result.success).toBe(false);
      expect(result.error?.type).toBe('not_found');
    });
  });

...
describe("PreferenceFilter", () => {
  ...
  it("computes cognitive load and filters by capacity", () => {
    const hard = makeItem({ chunkType: "new", difficulty: 8, easeFactor: 1.7, estimatedDuration: 25 });
    const easy = makeItem({ chunkType: "review", difficulty: 3, easeFactor: 3.2, estimatedDuration: 8 });
  ...
  it("estimates session duration with transitions", () => {
    const items = [makeItem({ estimatedDuration: 10 }), makeItem({ estimatedDuration: 10 }), makeItem({ estimatedDuration: 10 })];
  ...
  it("prioritizes overdue items first", () => {
    const overdue = makeItem({ id: "o", nextReviewDate: new Date(Date.now() - 86400000).toISOString().slice(0, 10) });
    const future = makeItem({ id: "f", nextReviewDate: new Date(Date.now() + 86400000).toISOString().slice(0, 10) });
  ...
  it("composes balanced session respecting constraints and max new items", () => {
    const items = [
      makeItem({ id: "o1", nextReviewDate: new Date(Date.now() - 86400000).toISOString().slice(0, 10), chunkType: "review", estimatedDuration: 10 }),
      makeItem({ id: "r1", nextReviewDate: new Date(Date.now() + 86400000).toISOString().slice(0, 10), chunkType: "review", estimatedDuration: 10 }),
      makeItem({ id: "n1", chunkType: "new", estimatedDuration: 10 }),
      makeItem({ id: "n2", chunkType: "new", estimatedDuration: 10 }),
    ];
    const out = composeBalancedSession(items, { maxDuration: 25, maxCognitiveLoad: 100, maxNewItems: 1 } as any);
  ...
  it("generates intelligent constraints based on time and patterns", () => {
    const out = generateIntelligentConstraints(45, { fatigueThreshold: 15, preferredDifficulty: 5 } as any);

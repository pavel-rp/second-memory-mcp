...
import { prerequisiteReferenceValidator } from "../../src/tools/prerequisite-reference-validator.js";
...
vi.mock("../../src/tools/prerequisite-reference-validator.js");
...
describe("PrerequisiteValidator", () => {
  ...
  describe("filterByPrerequisites", () => {
    ...
    it("should filter items with invalid prerequisite references", async () => {
      ...
      mockReferenceValidator.validateChunkPrerequisites.mockResolvedValue({
        ...
        errors: ["Chunk 'item1': Prerequisite reference 'invalid-prereq' does not exist as a chunk ID in the database"],
      });
  ...
  describe("validatePrerequisites", () => {
    ...
    it("should handle mastery check errors", async () => {
      mockMasteryService.checkItemMastery.mockRejectedValue(
        new Error("Mastery check failed")
      );

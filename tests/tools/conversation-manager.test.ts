...
describe("ConversationManager", () => {
  it("handles start learning with no items by prompting setup", () => {
    const cm = new ConversationManager();
    const out = cm.conductLearningSession({ intent: "start_learning", context: { learningItems: [] } } as any);
    expect(out.message.toLowerCase()).toContain("don't see any learning items");
    expect(out.needsInput).toBe(false);
  });

  it("produces guidance when items exist (guided)", () => {
    const cm = new ConversationManager();
    const out = cm.conductLearningSession({
  ...
  it("continues session and indicates final item when at end", () => {
    const cm = new ConversationManager();
    const out = cm.conductLearningSession({
  ...
  it("asks clarifying question for time", () => {
    const cm = new ConversationManager();
    const out = cm.conductLearningSession({ intent: "need_clarification", userInput: "how long should i study?" } as any);
    expect(out.needsInput).toBe(true);

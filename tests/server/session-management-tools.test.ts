...
describe("Integration: Session Management Tools", () => {
	...
	it("should create a session successfully", async () => {
		...
		const session = db.select().from(learningSessions).where(eq(learningSessions.id, parsed.sessionId)).get();
	...
	it("should complete a session successfully", async () => {
		...
		const session = db.select().from(learningSessions).where(eq(learningSessions.id, sessionId)).get();
	...
	it("should work with session chunks", async () => {
		...
		// Add session chunks
		db.insert(sessionChunks).values({

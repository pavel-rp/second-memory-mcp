...
describe("Validation Error Messages", () => {
	it("should provide clear error messages for title validation", () => {
		...
		if (!longResult.success) {
			expect(longResult.error.errors[0].message).toBe(`Title cannot exceed ${VALIDATION_CONSTANTS.MAX_TITLE_LENGTH} characters`);
		}
	});

	it("should provide clear error messages for difficulty validation", () => {
		const difficultySchema = z.number()
		...
		if (!lowResult.success) {
			expect(lowResult.error.errors[0].message).toBe(`Difficulty must be at least ${VALIDATION_CONSTANTS.MIN_DIFFICULTY}`);
		}
		...
		if (!highResult.success) {
			expect(highResult.error.errors[0].message).toBe(`Difficulty cannot exceed ${VALIDATION_CONSTANTS.MAX_DIFFICULTY}`);
		}
	});

	it("should provide clear error messages for quality score validation", () => {
		const qualitySchema = z.number()
		...
		expect(lowResult.success).toBe(false);
		if (!lowResult.success) {
			expect(lowResult.error.errors[0].message).toBe(`Quality score must be at least ${VALIDATION_CONSTANTS.MIN_QUALITY_SCORE}`);
		}
		...
		expect(highResult.success).toBe(false);
		if (!highResult.success) {
			expect(highResult.error.errors[0].message).toBe(`Quality score cannot exceed ${VALIDATION_CONSTANTS.MAX_QUALITY_SCORE}`);
		}
	});
	...
	it("should provide clear error messages for chunk type validation", () => {
		...
		if (!invalidResult.success) {
			expect(invalidResult.error.errors[0].message).toBe("Chunk type must be one of: new, review, remediation");
		}
	});
});

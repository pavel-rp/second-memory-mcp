...
describe('PrerequisiteReferenceValidator', () => {
  ...
  describe('validatePrerequisiteReferences', () => {
    ...
    it('returns valid when all IDs are empty/whitespace (uniqueIds empty after filter)', async () => {
      const result = await validator.validatePrerequisiteReferences(['', '  ', '   ']);
      expect(result.isValid).toBe(true);
      expect(result.validReferences).toEqual([]);
      expect(result.invalidReferences).toEqual([]);
  ...
  describe('caching', () => {
    ...
    it('uses cache when all requested IDs are cached and within expiry', async () => {
      let lookupCallCount = 0;
      const countingLookupFn = async (ids: string[]) => {
        lookupCallCount++;
        return new Set(ids.filter(id => existingIds.has(id)));
      };
      const countingValidator = new PrerequisiteReferenceValidator(
        countingLookupFn,
        lookupAllFn,
        () => NOW_MS
      );
      ...
      await countingValidator.validatePrerequisiteReferences(['chunk-1', 'chunk-2']);
      expect(lookupCallCount).toBe(1);

      // Second call with subset of cached IDs should use cache
      const result = await countingValidator.validatePrerequisiteReferences(['chunk-1']);
      expect(result.isValid).toBe(true);
      expect(lookupCallCount).toBe(1); // No additional lookup
    });

    it('falls through to lookup when cache misses some IDs', async () => {
      let lookupCallCount = 0;
      const countingLookupFn = async (ids: string[]) => {
        lookupCallCount++;
        return new Set(ids.filter(id => existingIds.has(id)));
      ...
      // Cache only chunk-1
      await countingValidator.validatePrerequisiteReferences(['chunk-1']);
      expect(lookupCallCount).toBe(1);

      // Request includes chunk-3 which is NOT in cache (cache only has chunk-1)
      await countingValidator.validatePrerequisiteReferences(['chunk-1', 'chunk-3']);
      expect(lookupCallCount).toBe(2); // Needed fresh lookup
    });

    it('skips cache update for large ID lists (>100)', async () => {
      let lookupCallCount = 0;
      const largeLookupFn = async (ids: string[]) => {
        lookupCallCount++;
        return new Set(ids);
      };
      const largeValidator = new PrerequisiteReferenceValidator(
        largeLookupFn,
        lookupAllFn,
        () => NOW_MS
      );
      ...
      await largeValidator.validatePrerequisiteReferences(manyIds);
      expect(lookupCallCount).toBe(1);

      // Second call should NOT use cache (cache wasn't updated for >100 IDs)
      await largeValidator.validatePrerequisiteReferences(['id-0']);
    ...
    it('does not use expired cache', async () => {
      ...
      const timedValidator = new PrerequisiteReferenceValidator(
        countingLookupFn,
        lookupAllFn,
        () => currentTime
      );
      ...
      // Warm cache
      await timedValidator.validatePrerequisiteReferences(['chunk-1']);
      expect(lookupCallCount).toBe(1);

      // Advance time past cache duration (5 minutes)
      currentTime = NOW_MS + 6 * 60 * 1000;

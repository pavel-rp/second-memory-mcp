...
`docs/research/C005-dp-map-schema/adjudication/01_schema-decision-ledger.md` was checked at adjudication time and carried **no `JS-E1` entry**, so no ruling had merged when these verdicts were drawn. Nothing here pre-empted it. Every reply states that if the finding is read as an attack on the severity rather than the prose, it is **ROUTED to NEU-950** and flagged as outside this slice's decomposed scope.

**The ruling has since landed, and it confirms this slice's restraint.** NEU-950 merged as PR 661 (`a3da473`) while this branch was open, adding ledger row **`D-R2`**. It rules the surmountability question **NO** — surmountability does not lower `JS-E1`'s own per-effect score — so `JS-E1` scores `blocking` and every single-effect `JS-E1` node keeps the severity it already carried. `D-R2`'s own evidence column names this file's nodes as the *precedent base that needs no correction*, citing `cl-2-combinatorial.yaml:2480-2481` and the live census at `:2303`, `:2480`, `:2547`, `:3025`, `:3387`, `:3639`, `:3734`. That is exactly the state this slice ships: had the prose fixes been allowed to drift into the severity, they would now be contradicting a merged ledger ruling. **Those citations survive this change**, because the one line this slice adds lands at 3864 — after every line `D-R2` cites — and every other edit is line-count-neutral.

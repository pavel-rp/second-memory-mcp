...
| 10 | **671** (deliverable) | `3707451978` | 88 | The new `ADDITIVE TOP-LEVEL KEYS` header says *"everything else at the top level is ADDITIVE"* but enumerates only a subset — internally inconsistent | **UPHELD** | Correct, and the defect was **this slice's own**, introduced by finding 9's fix in `9a1b1d6`. The file has **18** top-level keys and the note listed **14**, so *"everything else"* silently swept in `schema_version`, `cluster` and `map_version` — three keys that are not additive and that every sibling carries. Verified by counting `cl-2-combinatorial.yaml`'s exactly six top-level keys, which fixes the shared core at `schema_version` / `cluster` / `map_version` / `nodes`. This is finding 9's own failure mode in the opposite direction: over-claiming the scope is not a smaller warning, it is a different wrong one. | **FIXED HERE** — `3704234`. The note now states the arithmetic explicitly — 18 total, 4 shared and not in question, the other 14 enumerated — so 18 = 4 + 14 and a reader can check the claim without leaving the header. Still claims no key as ratified (SUB-2's and OUT-7's call) and still adds or removes no key. |
...
| `3704234` | The additive-key note's scope stated as explicit arithmetic (18 total = 4 shared + 14 additive) instead of an open-ended "everything else". Comment text. | 10 |
...
**A tenth finding landed on the deliverable pull request itself (#671)** and is included in the table above. It is recorded here rather than split into a separate document because it is a finding on **this slice's own fix** — the additive-key note added to close finding 9 over-claimed its scope. It was upheld, fixed in `3704234`, and replied to on its own thread. That the deliverable's own review caught a defect the instrument reviews did not is the system working as intended, and is reported rather than tidied away.

**Upheld and fixed: 8 of 10 findings** — and in five of the seven the fix was applied to the **whole defect class** (10 `mapper_note` blocks, 3 count sites, 9 path sites, 2 BigInt sites, 14 listed keys) rather than only to the lines the reviewer anchored. In each case the class was **enumerated by grep or by resolving paths on disk**, never assumed from the anchors.

**Rejected, file unchanged (2 of 10):** finding 1 (`JS-U5` is a registered code naming exactly this node's open question, and the file already cites a second registered code in prose at `:733`); and finding 2's *proposed formula*, which carried the opposite convention and would have contradicted `cl-4.smawk-application`.

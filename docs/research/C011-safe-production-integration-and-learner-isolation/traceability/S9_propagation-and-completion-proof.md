...
| --- | --- | --- | --- | --- | --- |
| OUT-12 | Six copy classes × three duties, every cell carrying an action, a deadline, a retention exception, a learner-visible result and an auditable proof | `../09_proving-a-data-right-reaches-every-copy.md` §7.1–§7.3 — 18 (class, duty) pairs as 17 rows (C4/C5 share one row under withdrawal; presentational only) | `derived` | **confirmed** | The cells are a design; none has been exercised. Execution is `R-S9-1` |
...
| OUT-12 | The sixth column carries the owner, retention bound and destruction condition SUB-1 recorded at position 1, with that origin named | §7.4, reading `../01_production-evidence-and-the-access-audit.md:151`–`:159` | `consumed` | **consumed** | The terms are unexercised — zero captures exist (`../01_…md:154`), so no term has been tested against a real capture. SUB-1 carries that as `R8` |
...
| OUT-12 | The unowned-copy audit runs mechanically over SUB-3's inventory ∪ `LD-S8-1` and reports a count | §8 — 33 categories audited, 28 mapping to a class, 5 deliberately unmapped with reasons, 0 with no propagation owner | `derived` | **confirmed** | The zero holds only within the inventory; **3** copy locations outside it are unclaimed — `F-S9-1`, `F-S9-5`, and the STDIO host state in `F-S9-4`. `R-S9-3` carries the risk that the qualification is dropped downstream |
| OUT-12 | Every copy the audit surfaces that no class claims is reported as a finding with an owner | `F-S9-1`, `F-S9-5`, and the STDIO host state carried in `F-S9-4` — **three** locations | `observed-in-repository` | **confirmed** | The provider's actual retention terms are unknown (`SPK-S9-1`, unexecuted); the container log driver's retention is unknown and is not claimed |
...
| OUT-12 | The copy set is **closed by an argument with a stated falsifier**, not assumed complete | §4.2 (`W-1a` … `W-8`) and §4.3; `../decision-records/DR-C11-S9-2_the-copy-set-closure-argument.md` | `observed-in-repository` | **confirmed** | Static at `ee0a750`. One new outbound client or one `writeFile` invalidates it — `R-S9-2` |
| OUT-12 | The deployment writes no learner data to disk **through a filesystem API** | §4.2 `W-2` — grep over `src/` for `writeFile` / `appendFile` / `createWriteStream` / `writeFileSync` / `mkdir` returned **zero matches** | `observed-in-repository` | **confirmed** | A measurement at one cutoff, not a standing property |
...
| OUT-12 | Propagation is demonstrated against a real copy | — | — | **not established** | **No production credential exists.** Zero designed spikes have executed — twenty-four at this branch's HEAD, twenty-two at cutoff `ee0a750` (`F-S9-2`). `G-S9-13` records this as `not met`; the §4 argument stands in its place and is **not** restated as a demonstration |

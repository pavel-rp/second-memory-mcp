...
| OUT-6 | Who may read it, and the explicit limit on what it entitles: `user` means *a `sub` claim was present*, **not** *a natural person*. | `DR-C11-S2-3` clauses 4–5; `../02_…md` §6 | `design` | confirmed | The humanity question is `A-S2-1`, carried by every consumer that reads the kind |

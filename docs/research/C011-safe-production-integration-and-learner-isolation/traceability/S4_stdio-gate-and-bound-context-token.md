...
columns additionally cite source and workflow paths (`src/…`, `drizzle/…`, `.github/…`,
`tests/…`, `docs/adr/…`), which are the excluded class the package's citation rules name and are
written bare from any depth. Chapter references are to
...
| OUT-7 | There is no STDIO transport module; the path is five inline lines (three statements) and both middlewares are Express-typed, so the gate is a rewrite rather than a mount | `../04_the-stdio-identity-gate-and-the-bound-context-token.md` §1.2; `F-S4-4` | Repository read — `src/transport/main.ts:55`–`:59`; `src/transport/audit-middleware.ts:23`; `src/transport/context-token-middleware.ts:43` | `confirmed` | The cost is unpriced by `CC-S8-3`'s classification; handed to `SUB-10 of C010 (NEU-984)` |
...
| OUT-13 | Mint, use, expiry, purge and cutover each carry a stated disposition on both transports and in the unconfigured case — 14 defined behaviours and one explicit `n/a` | `../04_the-stdio-identity-gate-and-the-bound-context-token.md` §6 | Design derivation | `confirmed` | The `n/a` is expiry on an unconfigured STDIO process, which mints nothing and so has nothing to expire |
| OUT-13 | The wired sweep is expiry-predicated and therefore **cannot** perform stage D's purge, which is a separate binding-predicated migration act | `../04_the-stdio-identity-gate-and-the-bound-context-token.md` §7, §9.1 stage D; `DR-C11-S4-3` clause 5 | Repository read — `src/adapters/drizzle/context-token-repository.ts:62` | `confirmed` | — |
| OUT-13 | A register-arithmetic discrepancy is raised rather than silently corrected: the spike register's cumulative total omits SUB-15's four entries | `../91_findings-register.md` § `F-S4-6`; `../96_spike-register.md` § SUB-4 totals | Repository read — the four `### SUB-<n>` sections of `../96_spike-register.md` | `confirmed` | Reconciliation is **SUB-14's** (NEU-1007); no sub-task edits another's section |
...
| OUT-13 | Four classes of token are rejected at cutover, the deploy pipeline's `client_credentials` smoke principal named explicitly and distinguished from the one-time unbound class | `../04_the-stdio-identity-gate-and-the-bound-context-token.md` §8; `F-S4-3`; `R-S4-2` | Repository read — `.github/workflows/cd-prod.yml:145`–`:174`; `tests/smoke/smoke.test.ts:195`, `:206`, `:237` | `confirmed` | The population of class C1 is **not observed** — `A-S4-1`, resolving through `OI-S1-7` / `SPK-S1-7` |
...
- **Any production quantity.** No production credential exists in this environment. The count of
  ...
  production are each carried as a stand-in or a spike, and the **`observed-in-production` evidence
  label is applied to zero claims** in this sub-task's output — named to record its emptiness,
  attached to nothing.

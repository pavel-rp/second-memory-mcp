...
| OUT-3 | The composition is **audited**, not asserted: ten constraints `K1` … `K10` with their sources, ten satisfied. | `../07_the-rollout-sequence-and-what-each-stage-cannot-undo.md` §2, §4 | Mechanical check against cited sources | confirmed | The audit is only as complete as the constraint set; a partial order no predecessor stated would not appear in it. |
...
| OUT-3 | The control class is the repository's **existing** toggle pattern, not an invented mechanism. | `DR-C11-S7-2` decision 2 | Codebase read: `src/config/resolve-classifier-config.ts:22`–`:62`; `docs/runbooks/classifier-blocking-activation.md:261`, `:167`, `:169` | confirmed | The precedent establishes the pattern works; it does not establish it works for these stages. |
...
| OUT-3 | **Exactly two of six smoke scenarios break, and both break twice** — established by reading the suite, not assumed. | `../07_the-rollout-sequence-and-what-each-stage-cannot-undo.md` §5 | Codebase read: `tests/smoke/smoke.test.ts:104`, `:111`, `:128`, `:152`, `:168`, `:206`, `:237`; `../05_the-enforcement-point-that-confines-every-read-and-write.md:338` | confirmed | Rests on `ContextTokenRepository` staying unscoped; if SUB-13 scopes it, `init_agent_context` becomes a third broken scenario. |

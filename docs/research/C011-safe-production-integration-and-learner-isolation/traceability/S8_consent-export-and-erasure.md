...
| OUT-11 | The artifact is **learner-readable, not a database dump**, against six stated properties | `../08_consent-and-what-a-learner-can-export-and-erase.md` §7.1 | `authored-position` | confirmed | **Capped by `../94_caps-and-incomplete-scope.md` § `CAP-S8-1`** — the table-top is a paper exercise; **no artifact was rendered**. |
...
| OUT-11 | **5** retention exceptions audited against all four fields; **4 pass, 1 fails**; **zero** indefinite accepted; the consent record audited as **one of them** | `../08_consent-and-what-a-learner-can-export-and-erase.md` §9 | `derived`, with each bound's enforcement mechanism cited or its absence stated | confirmed | Exception #4's 30-day bound is stated; its **cron registration exists only as a comment**, so enforcement is not asserted — `OI-S1-9`, cited. |

...
| `G-S11-6` | **`42` appears nowhere as a codebase fact**, and any citation resolving to a line 42 is disclosed. | **met** | §1.6 — the superseded numeral is named only as the thing superseded. **One citation in the chapter does resolve to a line 42** — `src/infrastructure/db/client.ts:42`, the pool's `max: 4`, cited in §5 and §8 — and is disclosed as a line number rather than claimed absent, which is the error `08_consent-and-what-a-learner-can-export-and-erase.md` §10.3 records having had to correct. |
...
| `G-S11-17` | No file under `src/`, `drizzle/`, `tests/` or any deployment configuration is modified. | **met** | §13 — `git diff --name-only origin/develop` lists files only under this package directory and `docs/GLOSSARY.md`. No test file is written; the four probes of §4.1 are specified and none is implemented. |
| `G-S11-18` | Every relative citation this sub-task writes resolves, the register appends remove zero lines, and the `…md` shorthand invisible to the checker is grepped for explicitly. | **met with cap** | `node_modules/.bin/tsx scripts/check-citation-paths.ts` run locally at this cutoff: C011 reports **0 non-resolving**. `git diff --numstat` reports a deletions count of **0** on all eight register files, re-checked after **every** edit pass. Every path reference in this sub-task's files is written as a full filename; the shorthand was grepped for explicitly, because `scripts/citation-paths/checker.ts:121` discards any candidate containing `…` **before it is counted**, so a green result is otherwise true but not evidence (`F-S5-7`, `G-S5-19`). **Capped by `CAP-S1-2`** (owner SUB-14): C011 is not in the checker's `GATED` list (`scripts/check-citation-paths.ts:20`–`:21`), so CI does not enforce it and the local run is the only gate. |
...
| `G-S11-21` | Any claim this sub-task cannot observe resolves to a stand-in, an owned open item, or a spike with a method and an expiry. | **met** | The unobservable claim is the existence and composition of the client population. It resolves to **`CAP-S11-1`** (the cap), **`SPK-S11-1`** (the bounded read-only experiment, designed and **not executed**, with a stated method and a 2027-02-25 expiry plus three event clauses) and, for the `R11` id premise, **`A-S11-1`**. **No claim in this sub-task carries the `observed-in-production` label**, and no client count is asserted in either direction. |

...
**This is the gate.** Positions 1–15 asserted, correctly, only what their own sub-task published, and
each said so: *"SUB-2 asserts only that **its own** rows are present and correctly disposed — not that
the gate as a whole is satisfiable."* This section answers the question they all deferred. The full
audit set is `98_audit-record.md`; the rows below cite it rather than restate it.

**Two things this section does not do.** It does not renumber, reflow or rewrite a single row above
it — the id-scheme divergence between flat `G-<n>` and scoped `G-S<n>-<k>` was adjudicated by SUB-14
as *"both stand, and neither is a defect"* (`00_method-and-provenance.md:340`), and this sub-task
agrees and changes nothing. And it does not repair any defect it finds: every one is routed to its
owning sub-task as a finding in `91_findings-register.md` § SUB-17.

**Rows use `G-S17-<k>`**, the scoped form, for the concurrency reason
`decision-records/DR-C11-S15-3_non-charter-register-id-scheme.md` gives — moot at this position, since
nothing runs concurrently with the last sub-task, and used anyway so the register ends on the
convention it converged to rather than reverting.

| # | Item | Disposition | Evidence / cap |
| --- | --- | --- | --- |
| `G-S17-1` | All eight registers are present in the reserved `90`–`99` band, and the audit set is published in it. | **met** | `90_` … `97_` all present, plus `98_audit-record.md`. `98_audit-record.md` §7. |
| `G-S17-2` | Every minted id appears as an entry in **exactly one** register. | **met** | **406 minted ids, `0` in more than one** — re-derived mechanically at `5f8e9cb`, reproducing SUB-14's figure. Every per-register count reproduces: 19 / 88 / 49 / 34 / 13 / 22 / 33 / 149 rows, **407 entries**. `98_audit-record.md` §7. |
| `G-S17-3` | Any id appearing in more than one register carries the same owner and status in each, with any conflict reported. | **met with cap** | **One divergence, declared by SUB-14 rather than discovered here** (`00_method-and-provenance.md:91`–`:105`): `F-S3-3`'s body in `91_` still reads live and `A-S11-1` carries no discharge record in `95_`, though both are resolved in `92_`. **Confirmed still open** at this cutoff. Capped as a **named residual** — its owners, SUB-3 and SUB-11, have shipped and this sub-task may not edit their entries. `F-S17-13`. |
| `G-S17-4` | The findings-register enumeration is reported in **both directions** over the outcomes the charter names. | **met** | **11 and 11.** Charter assumption 43 (`01_charter.md:645`) names OUT-2, OUT-3, OUT-4, OUT-8, OUT-9, OUT-10, OUT-11, OUT-12, OUT-14, OUT-17, OUT-18. Every named outcome carries the requirement in its own text; every outcome whose text carries it is named. SUB-14's correction of the decomposition's mis-attribution to assumption 49 is confirmed against the charter. `98_audit-record.md` §7. |
| `G-S17-5` | Every risk-register entry carries a severity, a mitigation, a named owner and an escalation route, and names its **authoring** sub-task rather than its aggregator. | **met** | **53 entries, `0` missing any of the four fields; `0` authored in a `### SUB-14` section.** Parsed mechanically. One matcher false positive against SUB-12's five entries was **declined on reading the file** — they use `**Owner.**` with a terminal period — and is recorded in `98_audit-record.md` §5 rather than reported as the subject's defect. |
| `G-S17-6` | All **fifteen** charter § Risks rows have an entry, each authored by the sub-task covering the owning outcome the charter names. | **met** | **15 / 15.** SUB-1 ×3, SUB-3 ×2, SUB-5, SUB-6, SUB-7 ×2, SUB-9, SUB-11, SUB-17 ×4 — the allocation `92_risk-register.md:40`–`:42` names. `R5`, `R6`, `R7` and `R15` are authored in `92_risk-register.md` § SUB-17. **`0` routed gaps.** The `F-S3-3` allocation is correct and independently corroborated: SUB-12 enumerated all fifteen owning outcomes from the charter alone at `92_risk-register.md:870` in the corrected order. |
| `G-S17-7` | Every `OUT-n` row carries its resolving evidence and its **success measure**, recorded by the producing sub-task, with **zero** measures authored at assembly. | **met** | **20 / 20**, OUT-20's row appended in `90_outcome-register.md` § SUB-17. All twenty carry a success measure and a measured result; **`0`** authored by SUB-14. Three measured results read NOT MET and none was restated to fit its outcome. `98_audit-record.md` §6. |
| `G-S17-8` | The split-fidelity audit reports **8/8** List B answered and **0** List A claimed. | **met** | `98_audit-record.md` §2 — every one of `H1`…`H8` mapped to its published chapter and outcome; `C1`…`C10` checked and none claimed. **`DR-C10-S5-2`'s revision trigger did not fire.** `H5` is answered by a registered stand-in (`A-S2-1`, charter assumption 35) rather than an observation, and that is stated at the count rather than smoothed. |
| `G-S17-9` | Every touched C010 residual id is classified into **exactly one** of four classes with its actual C010 owner named. | **met** | `98_audit-record.md` §3 — **26 ids: 16 from the charter's enumeration, 10 classified at assembly** under charter assumption 51's open-enumeration rule. **`0` in two classes; `0` classifiable into none.** `BND-S4-17` is resolved-here citing **`OI-S8-2`** at `:429`, never `OI-S8-1`. `CAP-S5-1` is owned-here, cites SUB-5's worked `holds` derivation, and is **not claimed lifted**. `OI-S5-3` is explicitly in none of the four. |
| `G-S17-10` | The citation gate covers this package and reports it clean. | **met** | **`CAP-S1-2` is discharged** — C011 is in the `GATED` list at `scripts/check-citation-paths.ts:23`. Run at this cutoff: **0 non-resolving, 0 exemptions claimed, exit 0.** **This supersedes `G-15` above**, which SUB-1 recorded `not met` at position 1 and which is now met; the append-only convention forbids editing that row, so the supersession is recorded here. **Three** rows above likewise read *"met with cap"* on `CAP-S1-2` — `G-S4-10` (`:144`), `G-S5-19` (`:231`) and `G-S11-18` (`:330`) — and are superseded with it; seven rows in total cite the cap under some disposition. |
| `G-S17-11` | The green gate is **not** taken as the evidence — all three checker blind spots are tested by hand. | **met** | `98_audit-record.md` §4.3–§4.4. (i) `…` shorthand discarded at `checker.ts:121` before counting; (ii) `MISSING-target` (`:247`–`:266`) can never fail — `isFailing` (`:342`–`:346`) reads only `inScope` and `staleNonClaims` — and is printed in neither output mode; (iii) hyphenated ranges unhandled at `:130`, **13 targets over 10 files**, reproducing SUB-14's figure exactly. The `MISSING-target` bucket was re-derived and read entry by entry: **265 occurrences over 76 distinct targets** at `5f8e9cb`, **zero unreported broken citations**. SUB-14's 235/73 was scoped to the assembly input and both are correct over their own corpora. |
| `G-S17-12` | Every codebase claim resolves to a real path at a stated cutoff. | **met** | **103 distinct codebase-path targets cited; 103 resolve; `0` do not** — the figure ranges over the six-prefix set (`src/`, `drizzle/`, `scripts/`, `tests/`, `docs/`, `.github/`) and excludes bare directories, one glob (`src/server/*.ts`) and ten repo-root files (`package.json`, `.env.example`, `docker-compose.yml`, `vitest.setup.ts`, …). **All ten repo-root files exist and were checked separately**, including `package.json`, read at the line below. Extracted mechanically and resolved against the worktree, because `src/` paths are an *excluded* class in the gate and a green run says nothing about them. Three load-bearing lines were read rather than trusted: `package.json:48`, `src/transport/main.ts:55`–`:59`, `src/transport/http.ts:176`–`:182` — all three confirmed as the package describes them. **This is one of the three classes that block the discharge declaration, and it is clear.** |
| `G-S17-13` | **`42` appears nowhere in the package as a codebase fact.** | **met** | Re-derived: `registerTool(` occurs **46** times across `src/server/`; `EXCLUDED_TOOLS` (`src/transport/context-token-middleware.ts:5`–`:9`) holds exactly three. **`42` is asserted as a tool-surface count in zero places** — every occurrence names it as the superseded miscount. **This is the second blocking class, and it is clear.** The package's separate line-number figure — a `:42` token 62 times across 27 merged files — is **published rather than denied**, which is the right call; one sentence overreaches it and is `F-S17-4`. |
| `G-S17-14` | No number in the package has quietly become a measurement. | **met** | **33 spikes designed, zero executed** — all 33 `Result` fields read *not executed* or record no observation, and all 33 entries now sit at one heading level so the two counting methods agree. **`observed-in-production` is applied to zero claims package-wide**; every one of its ~40 occurrences states the label's emptiness. `SMOKE_PROD_*`, `DATABASE_URL`, `AUTH_*`, `VPS_*` re-probed and unset. Every number is a cited derivation, a registered stand-in, or a deferred spike. |
| `G-S17-15` | The `A-28` handover and the `NEU-896` boundary are both stated, and the amendment routes exist. | **met** | `00_method-and-provenance.md` §8.1 (the trigger has fired; the envelope held; `CAP-S5-1` discharged not lifted) and §8.2 (the seam, two-sided and non-overlapping). The outbound routes are the ones `00_method-and-provenance.md` §8 collects; each destination named there was checked against the register entry that carries it, and **every one exists and names the destination claimed**. The per-id record is `98_audit-record.md` §3's classification table, which names a C010 owner for all 26 touched ids. Two carry a qualification worth stating: `DR-C11-S12-2` is a decision record and not a register entry, and C010's `OI-S8-1` route holds **only** while the id is written qualified, since C011 mints an unrelated `OI-S8-1` of its own. |
| `G-S17-16` | Every producing sub-task's acceptance conditions are asserted against this gate by their author. | **not met** | **`97_` carries the gate sections of eleven of the fourteen producing sub-tasks.** (Counting section headings alone now returns twelve, the twelfth being this `### SUB-17` section; the stable statement is the one scoped to producing sub-tasks.) SUB-15 and SUB-16 wrote none **by recorded decision** (`decision-records/DR-C11-S15-3_non-charter-register-id-scheme.md` consequence 6). **SUB-7 wrote none, and no decision records it anywhere** — `07_the-rollout-sequence-and-what-each-stage-cannot-undo.md` contains no reference to this register at all. `00_method-and-provenance.md:345`–`:347` states *"the **two** missing sections are a deliberate silence"*; eleven plus two is thirteen. **The fourteenth is SUB-7**, which covers OUT-3 and OUT-4 — the rollout and rollback outcomes, two List B questions and two Critical/High charter risks. **This row cannot be filled here**: writing gate rows for SUB-7 would author another sub-task's register content, forbidden for exactly the reason SUB-14 declined to write rows for SUB-15 and SUB-16. Registered as `F-S17-1`. **Owner: SUB-7 (NEU-1001)**, co-named SUB-14 and `NEU-896`. |
| `G-S17-17` | A QA pass exists for this package. | **not applicable** | The `qa-execution:engine` surface is unconfigured and the capability registry resolves to `git, linear`, so the autonomous QA phase is a genuine **Core Article 8 no-op** rather than a skipped gate. **No QA pass exists and none is claimed, for any sub-task.** The five `wf-audit` verify-phase lenses likewise did not fire, which is why an independent adversarial pass was run by hand over this sub-task's own output. Carried at package level by `README.md` § *"Verification note"*. **The id `CAP-S1-3` is not used for it**, and the reason has changed: `F-S17-2` establishes that the id is **not** a phantom — it is C010's cap, at `../C010-system-and-repository-architecture/91_caps-and-incomplete-scope.md:85`, about exactly this subject. |
| `G-S17-18` | The 67 DB-backed integration test files are run. | **not applicable** | They cannot run here — `vitest.setup.ts:6`–`:11` throws without `DATABASE_URL` and no database is provisioned. Recorded as **not run locally**, which is neither a pass nor a claim of unverifiability; they run in CI against a Postgres container, and this sub-task changes no file they cover. |
| `G-S17-19` | No file under `src/`, `drizzle/`, `tests/` or any deployment configuration is modified. | **met** | `git diff --name-only origin/develop` lists **six** paths, all under this package directory: `90_outcome-register.md`, `91_findings-register.md`, `92_risk-register.md`, `94_caps-and-incomplete-scope.md`, `97_package-completeness-gate.md` and `98_audit-record.md`. **Zero** under `src/`, **zero** under `drizzle/`, **zero** under `tests/`, zero deployment configuration. `.current-task` is **not** modified by this sub-task. `git diff --numstat` reports a deletions count of **0** on all six. *(An earlier revision of this row said "three paths" and omitted `90_`, `94_` and `97_` — the file the row is written in. Corrected after the adversarial pass.)* |
| `G-S17-20` | The independent cold read confirms the package is reconstructable from itself alone. | **met with cap** | See `98_audit-record.md` §8 and the row's cap there. |
| `G-S17-21` | Every finding this audit raises is routed to an owning sub-task and re-audited after repair. | **not met** | **Routed: yes — all twenty-one, each naming its owning sub-task.** **Re-audited: no, and it cannot be.** Every predecessor has shipped and this sub-task may not repair their content, so the route → repair → re-audit loop terminates at **round 1** with no repair possible. Under the convergence bound each becomes a **named residual cap** with the same stated reason. This is recorded as unmet rather than as a clean loop, because a loop that cannot run is not a loop that ran. `98_audit-record.md` §1.2. |

**SUB-17 rows: 21. Met 15; met with cap 2; not met 2; not applicable 2.**

*(An earlier revision of this line read "Met 16" and summed to 22 over 21 rows. It was caught by this
sub-task's own adversarial pass and is corrected rather than quietly restated — a gate whose row tally
does not sum is the exact defect class `F-S17-4`, `F-S17-10` and `F-S17-20` are filed against, occurring
in the tally written to close them.)*

**The two `not met` rows are the honest ones and neither is restated to match what was achievable.**
`G-S17-16` fails because a producing sub-task's gate section is absent and no remaining party may write
it. `G-S17-21` fails because the charter's convergence loop presumes re-openable predecessors and there
are none. Both were tested rather than assumed, both name what is missing, why, and its owner, and
**neither is narrowed to a condition it could pass** — the failure mode SUB-5 recorded against its own
`G-S5-6` and SUB-6 against its three.

**What SUB-17 does not assert here.** Nothing about whether any mechanism in this package **works** —
nothing has been applied, executed or observed, and the audit ranges over what the package *says*, not
over a running system. Nothing about whether `DR-C10-S5-1`'s five checks generate a complete universe;
the `8/8` above is 8/8 against the list as written, and `R5` carries that bound. Nothing about whether
`NEU-896` accepts the seam `00_method-and-provenance.md` §8.2 draws, which is `NEU-896`'s to accept. And nothing about the
correctness of the DDL, the rollout order or the threat model as **engineering** — the audit checks
that claims are supported, registered and routed, which is a different and weaker thing than checking
that they are right.

...
- **Risk:** A learner asks for erasure, the system reports success, and their data is still present somewhere. The charter names the two most likely somewheres: the log tables, and a backup whose existence is not established. This package has now added a third that the charter did not anticipate — **the copy that egresses to an external provider** (`F-S9-1`), which is worse than both, because the other two are at least inside a boundary this program can eventually reach. The failure is attractive precisely because it is invisible from inside: every mechanism reports what it did, none reports what it could not see, and a matrix with every cell filled looks identical to a matrix that is complete.
...
- **Mitigation status:** **Partially mitigated, and the residual is the point.** What is mitigated is every copy this deployment creates and controls. What is **not** mitigated: the egressed copy, which no mechanism in this repository can reach (`F-S9-1`); the backups class, whose contents are unestablished (`OI-S1-8`); and the execution of every action in the matrix, since this package applies nothing (`R-S9-1`). **No production observation supports any of it** — zero of twenty-two designed spikes have executed and `observed-in-production` is used zero times, so `R13`'s `n = 0` label applies in full to this entry.

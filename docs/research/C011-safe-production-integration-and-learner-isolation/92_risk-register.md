...
- **Severity:** **Medium** — the overlap window is small at the measured cadence, but the outcome of a partially applied concurrent migration on a database with **no established backups** (`OI-S1-8`) and **no down-migrations** is exactly the unrecoverable state `F-S15-1` says has no RTO.

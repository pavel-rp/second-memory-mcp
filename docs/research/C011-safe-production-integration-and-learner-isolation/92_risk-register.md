...
- **Mitigation:** Bound to verified platform facts rather than an idealized platform, and stated in four parts. **(1) Rollout is possible but not schedulable.** All ten stages are executable; none is executable at a chosen moment, because a merge to `develop` deploys whenever CI goes green and `OBJ-7` records ≥ 7 unannounced restarts per day. That is reported as `F-S7-5` rather than written around. **(2) Every rollback action was checked against a capability the deployment actually has.** Zero of the ten depend on an image registry, an IaC revert, a schema down-migration or a backup — forced rather than achieved, since SUB-15's tabletop found four of six recovery rows resolve to a capability the platform is not established to have, and the backups question is **`OI-S1-8`**, cited by id with no second record raised. `OBJ-13`/`OBJ-14` remain unset under `F-S15-1`. **(3) Containment is specified and is separately exercisable — and its precedent is weaker than it first appears.** Six of ten stages carry a deploy-independent disable path: an operator-set environment variable on the off-repo compose stack, applied over SSH. The in-repository `CLASSIFIER_ENABLE` toggle (`src/config/resolve-classifier-config.ts:22`–`:62`) establishes the **toggle shape**, but **not** deploy-independence — its own runbook's emergency path is *"1. Set `CLASSIFIER_ENABLE=false` … immediately"* followed by *"2. `Deploy.`"* (`docs/runbooks/classifier-blocking-activation.md:261`–`:262`), and no path in that runbook avoids a deploy. Applying the toggle over SSH to the off-repo stack, bypassing the pipeline, is therefore **new and unprecedented in this repository**. The remaining four stages carry a **named exception with a reason and an owner**; zero are blank. A pipeline-level all-or-nothing control also exists — disabling the `CD Prod` workflow — recorded as such and credited to no individual stage. **(4) Irreversibility is named, not discovered.** One stage, `T5`, is named irreversible; three stages' only reversal is a deploy and that is stated as the finding it is.
...
- **Risk:** `R-S4-2` offers three routes for the deploy pipeline's smoke run, and one of them is *"accept a known-failing step"*. If that route is chosen at stage `T0`, **the only automated post-deploy check the platform has** (`16_attribution-and-detection.md:146`) is red from `T6` onward — for the remaining four stages, including the backfill and the enforcement point going live. **Stated precisely:** the smoke run is *not* a limb of `SIG-S16-4`, which has exactly two limbs and neither is it; limb (b) is a database aggregate off `mcp_request_log.response_status` that a red smoke job cannot affect. What is lost is the post-deploy check, not limb (b). A subsequent genuine regression then arrives as one more red smoke run on a job that has been red for weeks, which is indistinguishable from the standing failure. The rollout would continue to be executable throughout, because `smoke-test` runs after `deploy-prod` (`F-S7-1`), so nothing would stop and nothing would signal.
...
- **Mitigation status:** **Open.** No route has been chosen — that is `OI-S7-1` — so no mitigation is yet in place; what exists is a decision point that has been made visible and given an owner. The residual, if the third route is chosen, is that stages `T6` through `T9` execute with the automated post-deploy check standing red and therefore uninterpretable — a genuine regression in it would be indistinguishable from the accepted failure. That residual's owner is the creator and it escalates to `NEU-896`. Note that the signal's *delivery* is separately unconfirmed in every case (`R-S16-2`), so this entry concerns whether the check is **interpretable**, not whether it is **routed** — and it concerns the smoke check specifically, not `SIG-S16-4`'s limbs, which are computed from the database and are unaffected.
...
*`NEU-1004`, covering `OUT-16`. **One charter `R<n>` row — `R11`**, computed from the charter alone
per the rule at `:17`, not from the forward-allocation table at `:33`–`:35`, which `F-S3-3` records
as permuting rows 10–12. The position and its derivation are stated at
`11_the-client-compatibility-contract.md` §10; **this sub-task does not resolve `F-S3-3`, which is
SUB-14's.** Two further entries are residual exposures this sub-task raises itself and take the
sub-task-scoped form `DR-C11-S15-3` fixes.*

## `R11` — The compatibility contract is written against a stale tool count and misses or invents a regression

- **Risk:** The compatibility contract is written against a stale tool count and misses or invents a regression. **This has already happened once:** a miscounted 42 propagated from C010's charter into `NEU-975`'s tracker description and from there into this charter's own intake, and the stale checkout this charter sits on reproduces the same number for an unrelated reason — it is behind `origin/develop` by PRs #686/#689 (`analytics_health`). **The two causes are independent**, which is what makes the number so durable: a reader who fixes the checkout still meets the miscount, and a reader who fixes the miscount still meets the checkout. `F-S8-1` sharpens the first cause into the form that actually bites — C010's figures were **not stale**; they were produced by running the verification procedure itself, seventeen days after the last change to `src/server/`, and still came out wrong. **A re-count is therefore not made trustworthy by being recent.**
- **Severity:** **High**
- **Owning outcome:** **OUT-16** — the backward-compatibility contract, which re-counts the surface at this package's own cutoff and records the branch it counted on.
- **Named owner:** **SUB-11 (NEU-1004)** for the re-count and the contract written on it, which is where a wrong number would enter this package; **the implementation charter `NEU-896` hands the work to**, for the surface as it stands at the landing cutoff, since the count is a property of a tree that moves and nothing here freezes it.
- **Escalation route:** **`NEU-895` (C010)**, which owns `F-S5-3` and `F-S8-1`: a re-count that contradicts 46 / 43 / 3 is evidence against a settled correction and routes a finding there rather than being adopted silently on either side. Co-named **`NEU-896`**. **No finding is routed by SUB-11 as things stand** — the re-count at `11_the-client-compatibility-contract.md` §1.4 **agrees** with `F-S5-3`, to the 41 / 1 / 1 granularity of where the declarations live and not merely on the three headline figures, so the routing condition did not arise. The route is recorded because it remains live for any later re-count, not because it fired.
- **Mitigation:** **Structural, and stronger than "count again".** Four limbs. **First**, the count is re-derived from `src/` at this sub-task's own cutoff (`35f92ba`, 2026-08-25, on a branch cut from `origin/develop` and therefore containing C010) and **never inherited** from the charter, the intake, a tracker description or a sibling chapter. **Second**, and directly against `F-S8-1`'s diagnosis, the derivation is published **disaggregated and re-runnable** — the exact commands, a sixteen-row per-module registration table, and a module-by-module mapping of the 43 `context_token` declarations onto the non-exempt registrations — so a reader re-runs it rather than trusting it. `46 − 3 = 43` is deliberately **not** how the gated figure is obtained; a subtraction would have reproduced the charter's own arithmetic error. **Third**, the exempt set is derived **twice by independent means** — empty input schema, and the middleware's hard-coded `EXCLUDED_TOOLS` — with their agreement reported as evidence and their lack of any structural coupling reported as a residual (`OI-S11-1`). **Fourth**, the durable claim is carried instead of the number: *the set of gated tools lacking `context_token` is empty* does not go stale the next time a tool is registered, and `43` does. Additionally, `42` appears nowhere in the chapter as a codebase fact; the one citation resolving to a line 42 is **disclosed** rather than claimed absent, and the one re-derived quantity whose total equals the numeral is written as a sum (`41 + 1`) so the total never appears. **The mitigation's own first draft breached this limb** — a bare `42` sat in the reconciliation table's *"Re-derived here"* column while the claim was certified met — and an adversarial pass caught it before the PR opened. That is recorded rather than repaired silently, because it is this very risk materialising inside its own mitigation, at a smaller scale, and it is the most direct evidence available that the four-limb mitigation needs its fourth limb. The precedent for disclosing rather than asserting an absence is `08_consent-and-what-a-learner-can-export-and-erase.md` **§11**, which records the same correction against its own earlier revision.
- **Mitigation status:** **Partially mitigated.** The number is right at this cutoff, by a derivation anyone can re-run, and it agrees with the settled correction on an independent path. What is **not** mitigated is durability: nothing in the repository detects the count changing. There is no lint rule, no CI check and no test asserting that the empty-schema set equals `EXCLUDED_TOOLS`, or that the `context_token` declarations map onto the non-exempt registrations — the derivation is prose in a research document, and a single new `registerTool` call falsifies the figure without touching this chapter. **Residuals, named:** the absent automated set-equality assertion, owned by **SUB-13 (NEU-1006)** under OUT-19 as the party writing the runbook's verification step, carried as `OI-S11-1`; and the re-count at the landing cutoff, owned by **the implementation charter `NEU-896` hands the work to**. This is the same shape as `R-S5-3` and is stated rather than borrowed.

## `R-S11-1` — The contract promises a reusable, non-DP-specific core over a surface that already is not one, and a self-hoster discovers it at integration time

- **Risk:** C005 `:61` requires core changes to be reusable, backward-compatible, **non-DP-specific** and fail safely, and OUT-16's contract is written against that clause. But `GradingPayloadShape` already hard-codes four dynamic-programming criterion keys into two core tool input schemas and one response payload, reaching 3 of the 46 registered tools (`F-S11-2`). The exposure is not that this package breaks the clause — it does not — but that **publishing a compatibility contract over the surface implies the clause holds of the surface**, and it does not. A second operator adopting Second Memory as the general-purpose MCP server it claims to be meets `correct_recurrence` as a **required** boolean on `submit_answer` at integration time, with no way to omit it and nothing in the published contract having warned them.
- **Severity:** **Medium** — it is not an isolation, privacy or correctness defect, and it breaks nothing that works today, because today's only client is the first-party DP course for which the rubric is correct. It is not Low because *reusable, non-DP-specific core* is a **charter-level constraint**, the whole premise on which this server is separable from its first application, and this is the outcome whose job is to check it.
- **Owning outcome:** **OUT-16** — the compatibility contract and its DP-specificity review, which is where the clause is checked and therefore where an unstated breach would first become a false promise.
- **Named owner:** **`NEU-896`** at convergence, as the party that decides whether a core-reusability breach is fixed, accepted or scoped, since no charter currently in flight owns `src/domain/types/teaching.ts` and this package may not change `src/` by constraint; **the creator, as sole maintainer**, for the surface itself.
- **Escalation route:** **`NEU-896`**, which owns cross-package reconciliation and the go / conditional-go decision, and is the only party positioned to weigh a charter-level constraint against a shipped first-party schema. **Not `NEU-895`** — this is measured against `DR-C10-S8-1`'s `R8-4` rule rather than disputing any C010 record, so no amendment is routed there.
- **Mitigation:** **Disclosure only, and stated as such.** The breach is named at `11_the-client-compatibility-contract.md` §9 with all six of its locations cited, rather than left for a second operator to find; the review's two verdicts are reported **separately** — clean on this package's seven changes, breached on the pre-existing surface — so neither is read as the other; and the finding is scoped explicitly as a **reusability** defect rather than an isolation or privacy one, so it is not escalated past what it is. What is **not** mitigated is the breach itself: no remedy is proposed, because proposing one would be deciding a `src/` change this sub-task may not make, and because the reasonable remedies (a generic criterion map, a per-course rubric registry, moving the rubric to the application) differ enough that naming one would foreclose the others.
- **Mitigation status:** **Open.** No remedy exists and none is proposed. **Residual, named:** the breach itself, owned by **`NEU-896`**; and the absence of any check that would catch the *next* course-specific concept entering the core surface — there is no lint rule and no review gate, and this review is a one-off at one cutoff. **No claim is made about how many self-hosters exist**; `CAP-S11-1` states that the population is unobserved and `R13`'s `n = 1` position applies in full.

## `R-S11-2` — The gate extraction is cut as "just a mount", and the seven-path table is read as still describing what shipped

- **Risk:** `F-S4-4` establishes that reaching STDIO is a rewrite rather than a mount, and `R-S4-4` already carries the hazard that work priced as a mount and discovered to be a rewrite is the work most likely to be cut. **This entry is the distinct downstream exposure**: if the extraction is cut, SUB-4's seven-path classification — the artifact a rollout planner will actually consult — silently stops describing reality. Three of the seven rows re-read as **unaffected**, the same word the genuinely-unaffected row carries, while what has actually happened is that `CH-5` … `CH-7` shipped at the adapter and every STDIO caller is refused as kind `none` with no transport gate to name the reason (`F-S11-4`). A planner scanning the column sees five unaffected rows and concludes STDIO came through well.
- **Severity:** **Medium** — the underlying refusal is safe rather than exposing, so nothing leaks, and the condition is recoverable once observed. It is not Low because the misreading lands on **path 4, the largest class** — everything launched with `TRANSPORT` unset, which is the default (`src/config/resolve-transport-config.ts:35`) — and because the symptom (a refusal from inside the adapter, unlogged on STDIO for want of audit parity) is close to maximally hard to attribute to its cause.
- **Owning outcome:** **OUT-16** — the compatibility contract, which is where the seven paths are folded into the whole-surface statement and therefore where a tier-dependent reading of them originates.
- **Named owner:** **SUB-7 (NEU-1001)** under OUT-3, whose stage sequence determines which of the three tiers actually ships and which is therefore the only party positioned to prevent the mismatch; **`SUB-10 of C010 (NEU-984)`**, co-named **`NEU-896`**, as `CC-S8-3`'s owner and the party the extraction itself belongs to.
- **Escalation route:** **`NEU-896`** at convergence, as the party that decides whether a transport may ship confined-but-unexplained, which is a program-level operability question rather than a defect this package can close; **additionally `SUB-10 of C010 (NEU-984)` directly**, because the extraction's scope is the thing that determines the tier.
- **Mitigation:** **By disclosure and by construction of the artifact, not by measurement.** The three tiers are tabulated against all seven paths at `11_the-client-compatibility-contract.md` §6.3, so the tier-dependence is visible in the same table a planner reads rather than inferable from prose; the word *unaffected* under Tier N is explicitly annotated as *unaffected by the gate, not unaffected in outcome*; the extraction's **option A / option B** fork is named at §6.2 so the piece that actually sizes the work is visible before it is scoped; and §5 records that audit parity is **not** required for `I4`, so a green `I4` cannot be cited as the argument for cutting the limb whose absence causes the unattributable symptom. What is **not** mitigated is the decision: nothing here can prevent the extraction being cut, and no effort estimate is offered that would let anyone judge whether cutting it is reasonable.
- **Mitigation status:** **Open.** No tier is chosen; the sub-task that sequences the stages runs at position 13 and the extraction's owner is outside this package entirely. **Residual, named:** the tier choice, owned by **SUB-7 (NEU-1001)**; the extraction's scope and cost, owned by **`SUB-10 of C010 (NEU-984)`**; and the audit-parity limb, already owned as **`R-S4-4`** by SUB-4 and **cited here rather than restated**, since this package carries one id per fact.

---

**SUB-11 register totals at revision 1:** three entries — **`R11` (High)**, `R-S11-1` (Medium),
`R-S11-2` (Medium). **Exactly one charter `R<n>` row**, correctly: charter § Risks row 11 is the
only one of the fifteen naming OUT-16 as its owning outcome, per charter assumption 48. Two open,
one partially mitigated; every non-mitigated status names its residual and that residual's owner,
and every entry carries a severity, a mitigation, a named owner and an escalation route.

**On the id, and what is deliberately left unresolved.** `R11` is computed from the charter's own
§ Risks table, read at this sub-task's cutoff, per the rule stated at `:17` and in `README.md`. The
derivation and its cross-check against seven already-claimed ids — `R1`, `R8`, `R9`, `R10`, `R12`, `R13`,
`R14` — all seven of which agree with charter position rather than with the forward-allocation table —
are at `11_the-client-compatibility-contract.md` §10. **`F-S3-3` is cited, not resolved.** Whether
the table at `:33`–`:35` should be corrected, and whether the two sources disagree because the
charter moved or because the table was written wrong, are **SUB-14's** to settle; `_local/` is
gitignored and unversioned, so the two readings cannot be diffed from here. SUB-1's entries are
untouched, and nothing here reflows, renumbers or rewrites them. The assumption this rests on — that
the § Risks table has not been reordered since SUB-3's read — is registered as `A-S11-1` rather than
left implicit.

**One risk is deliberately not raised here, because it is already recorded exactly once elsewhere.**
That the deploy pipeline's smoke run is refused on every release is **`R-S4-2`** (OUT-13, SUB-4),
with `F-S5-12` recording the second, independent cause. This sub-task adds only a **scenario-level
scoping** of that break — six of eight smoke scenarios pass, two fail, both row-owning
(`11_the-client-compatibility-contract.md` §7) — which sharpens an existing entry rather than
opening a competing one against a hazard SUB-4 already owns.

---

### SUB-9

**Id shape in this section.** One charter row — **`R2`**, the charter's § Risks row 2, the only one
of the fifteen naming **OUT-12** as its owning outcome, which makes this sub-task its named author
under charter assumption 48. Three further entries use the sub-task-scoped `R-S9-<k>` form. The
allocation matches this register's own id-convention table.

## `R2` — Erasure or withdrawal completes on paper while learner data survives in a copy nobody owns

- **Risk:** A learner asks for erasure, the system reports success, and their data is still present somewhere. The charter names the two most likely somewheres: the log tables, and a backup whose existence is not established. This package has added **two more the charter did not anticipate**, and both are worse than the named ones because the named ones are at least inside a boundary this program can eventually reach: **the copy that egresses to an external provider** (`F-S9-1`), and **the copy of both log tables that the shared pino sink writes to stderr** (`F-S9-5`) — the latter surviving every action in this outcome's own matrix, including the bulk disposal designed to close the log-table gap. The failure is attractive precisely because it is invisible from inside: every mechanism reports what it did, none reports what it could not see, and a matrix with every cell filled looks identical to a matrix that is complete.
- **Severity:** **Critical** — one of the charter's three. It is a silent, systematic failure of a duty the package exists to discharge, and the party told it succeeded is the learner.
- **Owning outcome:** **OUT-12** — the propagation-and-proof matrix, which is where a copy without an owner becomes visible.
- **Named owner:** **SUB-9** (NEU-1003) for the matrix, the copy-set closure and the pre-cutover disposition. **The creator, as sole maintainer and sole operator**, for executing any disposal and for every fact about the production deployment the package could not observe.
- **Escalation route:** **`NEU-986`** (`SUB-12 of C010`), owner of `CAP-S3-3` and `CAP-S4-1` — the retention-and-deletion caps over the two log tables, for which this package supplies the mechanism (`09_proving-a-data-right-reaches-every-copy.md` §6.6) but which it does not own — **co-named `NEU-896`**. **Where the unresolved copy is a backup rather than a log table**, the route is the owner carried on `OI-S1-8`, the package's single record of that fact: the creator, as sole maintainer and sole operator (`93_open-items-and-provisional-register.md:124`). **Where the unresolved copy is the external-provider egress**, the route is `NEU-896` at convergence, per `F-S9-1` — a third party's retention terms are not this repository's to set.
- **Mitigation:** The matrix admits no cell reading "unknown" without an owner and a date, and three C3 cells carry exactly that owner and date rather than a blank. **The copy set is closed by an argument with a stated falsifier** (`DR-C11-S9-2`) rather than by a survey, which is the only part of this mitigation that addresses the failure's actual mechanism — a survey cannot find the copy nobody thought of, and the write-path enumeration is falsifiable by four greps and no production access. The pre-cutover population, the charter's own most-likely somewhere, is **disposed of rather than deferred** (`DR-C11-S9-1`), which discharges `F-S8-2`. The retention-and-deletion mechanism for the log tables is designed and handed to `NEU-986` with a **code-derived** 5-week floor rather than a guessed one. The unowned-copy audit is driven mechanically over 33 categories and reported as a count. The package's own copies are inside the matrix, not exempt from it. And the one copy the audit surfaced that no class claims is **reported as `F-S9-1` with an owner** rather than absorbed — which is what keeps "no unowned copy" falsifiable instead of a restatement of the matrix.
- **Mitigation status:** **Partially mitigated, and the residual is the point.** What is mitigated is every copy this deployment creates and controls **through a store the matrix can address**. What is **not** mitigated: the egressed copy, which no mechanism in this repository can reach (`F-S9-1`); **the stderr log copy, which mirrors both log tables and which every action in this matrix bypasses entirely (`F-S9-5`)**; the backups class, whose contents are unestablished (`OI-S1-8`); and the execution of every action in the matrix, since this package applies nothing (`R-S9-1`). **`F-S9-5` is the most important of the four and the least anticipated** — it is this risk's own failure mode occurring inside the artifact written to prevent it, and it was found only by re-attacking an enumeration that had already passed its checks. **No production observation supports any of this** — zero designed spikes have executed — twenty-four at this branch's HEAD, twenty-two at cutoff `ee0a750` (`F-S9-2`) — and `observed-in-production` is used zero times, so `R13`'s `n = 0` label applies in full to this entry.

## `R-S9-1` — The disposal is designed and never executed, so the pre-cutover population survives the package that disposed of it

- **Risk:** `DR-C11-S9-1` publishes a disposition; it does not run one. This package applies nothing — no DDL, no migration, no production access — so the pre-cutover rows are still there when the package publishes, and `F-S8-2` is recorded **resolved** while the population it names is intact. A reader who takes "resolved" for "done" will believe an erasure duty is discharged that is not.
- **Severity:** **High** — it is the exact shape of `R2` applied to this sub-task's own output, and it is created by this sub-task's own success. A published disposition is more likely to be mistaken for a completed one than an open finding is.
- **Owning outcome:** **OUT-12**.
- **Named owner:** **The creator, as sole maintainer and sole operator** — the only party who can execute a deletion against the production database. The design obligation is SUB-9's and is discharged; the execution is not SUB-9's to take.
- **Escalation route:** **`NEU-896`** at convergence, which hands the implementation-ready artifacts to the charter that actually executes them; a disposal never performed is a program-level residual, not a defect this package can close. Co-named **`NEU-986`** where the unexecuted disposal is the log-table retention mechanism specifically.
- **Mitigation:** The distinction is stated in four places rather than implied once — `09_…md` §6.5 and §12, `DR-C11-S9-1` consequence 1, and the outcome register's *"what this measure does not claim"*. `F-S8-2`'s downgrade is written as *resolved on its stated resolving event*, with that event quoted, so a reader can see exactly what was and was not achieved. The disposal carries a **dated bound** (clause 5) rather than a policy, so an unexecuted disposal is visibly overdue rather than indefinitely pending.
- **Mitigation status:** **Partially mitigated.** The statements are in place and the bound is dated. The residual is that they are statements — the only thing that closes this is the deletion running, and no party inside this package can run it. This is the same shape `R-S6-1` took against SUB-6, one step further down the chain, and it is recorded rather than passed on silently.

## `R-S9-2` — The copy-set closure argument is static, and one new line of code invalidates it without anyone noticing

- **Risk:** `DR-C11-S9-2`'s argument is a measurement over `src/` at cutoff `ee0a750`. Its strongest single result — that there are **zero** filesystem writes anywhere in `src/` — is invalidated by one `writeFile` call. Its egress enumeration is invalidated by one new outbound client. Nothing in CI checks either, so the argument can become false while continuing to read as true, and every completeness claim resting on it degrades silently.
- **Severity:** **Medium** — it does not create an exposure; it creates a claim that stops tracking reality. The underlying data does not become less safe, but the package's statement about it becomes wrong.
- **Owning outcome:** **OUT-12**.
- **Named owner:** **SUB-9** (NEU-1003) for stating the argument as a count at a named cutoff rather than as a standing fact; **`NEU-896`** for whether a standing check is worth building.
- **Escalation route:** **`NEU-896`** at convergence. A CI check over egress and filesystem writes is a repository-wide control, not a C011 artifact, and this package writes no `src/` or CI code by constraint.
- **Mitigation:** The argument is written as **falsifiable by construction** — the falsifier is stated explicitly (`09_…md` §4.3), the method is four greps, and the revision trigger in `DR-C11-S9-2` names both invalidating events. The cutoff is carried in the chapter's front matter and repeated at every enumeration, following the convention SUB-4 set and SUB-5 restated for the spike count: a count is only correct on the day it is taken.
- **Mitigation status:** **Partially mitigated.** Discoverability is handled; enforcement is not. The honest position is that this argument needs re-running at any later cutoff that cites it, and the package says so rather than presenting the enumeration as permanent.

## `R-S9-3` — The matrix's completeness claim is qualified, and the qualification is the first thing a downstream reader will drop

- **Risk:** `09_…md` §8 reports **zero** unowned copies within the inventory. That figure is true only under the bound in §4.4 and §9 — it ranges over copies this deployment *creates and controls*, and explicitly not over the external-provider egress. A downstream consumer quoting "zero unowned copies" without the bound states something false, and the number is far more quotable than the qualification attached to it. This is how the miscounted `42` propagated from C010's charter into a tracker description and back into this charter's intake.
- **Severity:** **Medium** — it is a transmission risk rather than a design defect. The package is correct; what leaves it may not be.
- **Owning outcome:** **OUT-12**.
- **Named owner:** **SUB-9** (NEU-1003) for binding the figure to its qualification at every occurrence; **SUB-14** (NEU-1007) under OUT-20, which assembles the package a cold reader receives.
- **Escalation route:** **`NEU-895`** (C010) is **not** the route — nothing here contradicts a C010 decision. **`NEU-896`** at convergence, where a cross-package quotation of this figure would surface.
- **Mitigation:** The zero is never written alone. Every statement of it in `09_…md` §8 carries the count of unclaimed locations (**1**) in the same table, and the audit's result row for unowned copies sits directly above the row for copies no class claims, so the two cannot be read apart. §9 states the bound as a section rather than a footnote, and the outcome register's success measure repeats it in *"what this measure does not claim"*. `F-S9-1` gives the exception its own id, so a reader who meets the number downstream has something to look up.
- **Mitigation status:** **Partially mitigated.** Inside this package the figure and its bound are inseparable. Outside it, nothing prevents the number being lifted alone, and the precedent that this happens is `F-S8-1`'s diagnosis of the `42`.

---

**SUB-9 register totals at revision 1:** **four entries** — one charter § Risks row (**`R2`**, Critical, OUT-12) and three sub-task-scoped (`R-S9-1` High, `R-S9-2` Medium, `R-S9-3` Medium). Every entry carries a severity, a mitigation, a named owner and an escalation route, and **not one is fully mitigated** — each records what remains and who holds it.

**One predecessor's residual is closed by this section, without editing that predecessor's entry.** `R-S6-1` records that *"the only thing that actually closes it is SUB-9 publishing its disposition"* and that *"that residual's owner is SUB-9"* (`:586`–`:588`). The disposition is published in `DR-C11-S9-1`, and it is **not** "accepted residual" — so the specific misreading `R-S6-1` guards against, an archive persisting with the duty attached, does not arise. **`R-S6-1`'s text is left exactly as SUB-6 published it**; this register is append-only, and the closure is recorded here and routed to **SUB-14** (NEU-1007) to reflect at assembly.
...
- **Mitigation.** §5.2 states the scope explicitly as a standing qualifier rather than a footnote; every operator path is enumerated (`TP-S12-35` … `TP-S12-43` and `TP-S12-54` … `TP-S12-56`) with an invariant, so the scope boundary is visible as twelve concrete rows rather than as a caveat; `GATE-S12-15` sets a control on the one path (`psql`) that most looks like an exemption. **The mitigation is explicitly not complete** — a port-boundary mechanism *cannot* reach below the port boundary, and placing it there is what `A-28`'s envelope requires. What is mitigated is the silence, not the scope.
...
- **Severity:** Medium
- **Risk.** The ingress partition is read off one conditional at one cutoff — `src/transport/main.ts:46`–`:59`. The STDIO arm already demonstrates the failure mode: it exists, it carries none of the middleware the HTTP arm mounts, and that asymmetry is the reason `F-S5-4` reports every category failing at `I4`. **A third arm would inherit the same default**, and the threat model would report 56 paths over a system that had 57.
...
- **Risk:** All six control variables `DR-C11-S13-3` publishes — four behaviour toggles and two numeric sweep parameters, of which only the toggles are disable paths — are environment variables on the compose stack at `/home/deploy/docker-services/second-memory-mcp`, which is **outside this repository** (`.github/workflows/cd-prod.yml:15`, `:26`–`:30`). Applying one requires SSH to that host, which is a capability only the creator holds. So the containment step that stands between detecting a regression and reverting a deploy — the whole point of OUT-4's containment-versus-reversal split — is unavailable to anyone else, at any hour, for any stage. A rollout with one operator has no containment at all whenever that operator is unreachable.
- **Severity:** **High** — it does not make a failure more likely, and it makes every failure's containment window depend on one person's availability, across a ten-stage sequence in which one stage is irreversible and **three cannot be entered from their predecessor's disable position** — `T5`, `T8` and `T9`, SUB-7's own enumeration at `07_the-rollout-sequence-and-what-each-stage-cannot-undo.md:529`, cited rather than re-derived.
...
**The four rows the charter owns to OUT-20, authored here.** `R5`, `R6`, `R7` and `R15` were reserved
slots at assembly (`00_method-and-provenance.md:189`–`:199`); SUB-14 authored no register content, so
their content is this sub-task's. With these four the register carries **53 entries** and all fifteen
charter § Risks rows are authored — eleven by their producing sub-tasks, four here.

## `R5` — The inherited 18-question universe is only as complete as `DR-C10-S5-1`'s five checks, and this package did not re-derive it

- **Risk:** C010's split universe is **18** questions, partitioned 10 on List A and 8 on List B, and its own audit records that the two zeros — *"on both lists"* and *"on neither list"* — are **definitional rather than empirical**, because the universe *is defined as* List A ∪ List B (`../C010-system-and-repository-architecture/06_isolation-invariant-and-the-neu-893-split.md:580`–`:583`). The universe was generated by walking `DR-C10-S5-1`'s five ordered checks. **A failure mode those five checks cannot generate produces a question that appears on neither list and that no audit on either side will report as missing.** C011 consumed the partition and deliberately did not re-derive it, so this package's own split-fidelity result (§2 of `98_audit-record.md`, 8/8 and 0) inherits exactly that bound.
- **Severity:** **High**
- **Owning outcome:** **OUT-20** — the audit outcome, which owns the split-fidelity record and is where the two zeros are reported.
- **Named owner:** **SUB-17 (NEU-1008)**, this sub-task, which reports the bound rather than closing it. No party inside C011 can close it: re-deriving the universe means re-running C010's check set against C010's matrix, which is C010's artifact and outside this package's scope by constraint.
- **Escalation route:** **`NEU-895`**, which owns `DR-C10-S5-1`, **co-named `NEU-896`** at convergence, where a completeness claim resting on another package's generator is reconciled.
- **Mitigation:** The bound is stated wherever the count is, rather than only here: `98_audit-record.md` §2 reports 8/8 and 0-List-A **with** the definitional caveat attached to the same sentence, on the precedent C010 itself set by putting the caveat where a reader stops. The amendment route is named and open — a failure mode the five checks cannot generate is routed to `DR-C10-S5-1` as a recorded amendment rather than absorbed.
- **Whether it fired: it fired.** This is the clause the charter requires be reported rather than assumed. **SUB-12 found two failure modes `DR-C10-S5-1`'s five ordered checks do not generate** — cross-learner actuation (`F-S12-1`) and a verdict set with no over-confinement outcome — firing that record's trigger 3. The amendment is `decision-records/DR-C11-S12-2_the-unconfined-aggregate-as-a-control-input.md`, routed to `NEU-895` co-named `NEU-896`, with **SUB-17 named as the recipient within this package** (`92_risk-register.md:924`). **This sub-task confirms receipt and reports it as fired.** It is the package's only genuine contradiction of a C010 decision; twelve chapters ran their C010 checks clean before it.
- **Mitigation status:** **Open.** The route fired and the amendment is routed, which is the mechanism working, **not** the risk closing. The universe is still un-re-derived, the two zeros are still definitional, and a second undetectable question would still be undetectable. **Residual, named:** the completeness of `DR-C10-S5-1`'s five checks, owned by `NEU-895`, unresolved at this cutoff and not resolvable here.

---

## `R6` — OUT-19's implementation-ready artifacts and `NEU-896`'s remit collide, and two packages each believe they own the handoff

- **Risk:** OUT-19 publishes DDL, a migration plan and a runbook at *"a fidelity an implementer executes without asking a question"* (`90_outcome-register.md` § OUT-19), while `NEU-896`'s remit is *"implementation-ready program packages"*. The two descriptions overlap, and the failure is symmetric: either both parties assume the other converges the handoff and nobody does, or both do and a divergence between two convergences reaches an implementer as two conflicting instructions.
- **Severity:** **High**
- **Owning outcome:** **OUT-20** — the audit outcome, which is where the boundary is checked rather than assumed.
- **Named owner:** **SUB-17 (NEU-1008)**, which audits that the boundary statement exists and states a seam; it does not author the boundary, which is SUB-14's.
- **Escalation route:** **`NEU-896`** as the convergence gate, which owns the go / conditional-go decision and is the only party that can resolve an overlap in its own remit.
- **Mitigation:** SUB-14 published the boundary as an explicit seam at `00_method-and-provenance.md` §8.2 — an enumerated *hands over, finished* list against an enumerated *`NEU-896` still owns* list — and stated that *"the seam is a boundary, not a claim"*, with `CAP-S13-1` and `CAP-S7-1` cited as the reason nothing in OUT-19 is implementation-ready in the sense `NEU-896` means. **This audit checked the seam and found it present and two-sided**, with no item appearing on both lists — the seam itself is `00_method-and-provenance.md` §8.2, and the check is recorded at `97_package-completeness-gate.md` § SUB-17 `G-S17-15`.
- **Mitigation status:** **Partially mitigated.** The seam is published and checked, which removes the *silent* form of the collision. What is not mitigated is that the boundary is stated by one side only — C011 — and has not been agreed by `NEU-896`, which has not yet run. **Residual, named:** `NEU-896`'s acceptance of the seam as stated, owned by `NEU-896`. A boundary one party draws is a proposal until the other reads it.

---

## `R7` — The package's scope drifts from List B, and the split contract breaks silently

- **Risk:** `DR-C10-S5-2`'s revision trigger fires if *"NEU-893 publishes a scope that does not match List B — it declines a question on it, or claims one from List A"* (`../C010-system-and-repository-architecture/decision-records/DR-C10-S5-2_the-neu-893-split-contract.md:132`). Drift is silent by construction: a declined List B question looks like a question that was never in scope, and a claimed List A question looks like thoroughness. Neither is visible without a mechanical audit against the two lists as written.
- **Severity:** **High**
- **Owning outcome:** **OUT-20** — the audit outcome, which owns the split-fidelity record where the counts are reported.
- **Named owner:** **SUB-17 (NEU-1008)**, this sub-task, whose split-fidelity audit is where 8/8, 0-List-A and the residual-id classification are reported.
- **Escalation route:** **`NEU-895` (C010)**, co-named **`NEU-896`**, because a declined List B question or a claimed List A question is the disagreement `DR-C10-S5-2`'s revision trigger names and is settled **in that record**, not absorbed here.
- **Mitigation:** The mechanical audit itself. List B is taken verbatim from `../C010-system-and-repository-architecture/06_isolation-invariant-and-the-neu-893-split.md:531`–`:545` and List A from `:514`–`:529`; each of `H1`…`H8` is mapped one-to-one onto the published chapter and outcome that answers it, and each of `C1`…`C10` is checked for a C011 claim. Every touched C010 residual id is classified into exactly one of four classes with its actual C010 owner named. Counts are reported, not impressions.
- **Result at revision 1:** **`8 / 8` List B questions answered, `0` List A questions claimed, `0` ids in two classes, `0` ids classifiable into none.** The trigger did **not** fire. Reported in full at `98_audit-record.md` §2 and §3.
- **Mitigation status:** **Mitigated at revision 1, with one bound named.** The audit ran and returned clean on all four measures. **Residual, named:** the audit's own reach is `R5`'s — a List B question that `DR-C10-S5-1`'s five checks never generated would be absent from the list this audit checks against, and 8/8 against an incomplete list is 8/8 against an incomplete list. Owned by `NEU-895`. The clean result is therefore evidence that the published scope matches the contract **as written**, not that the contract is complete.

---

## `R15` — The package's vocabulary collides with the domain's own terms

- **Risk:** `subject`, `session` and `context token` each already mean something specific in this codebase, and each is used in this package in a second sense. `subject` is a JWT claim here and an academic subject in the product domain; `session` is a learning session in the domain and a database session in the threat model; `context token` is a product concept and a `context_tokens` row. A reader who takes the wrong sense at first occurrence misreads the mechanism, and the failure is silent because both readings are locally coherent.
- **Severity:** **Low**
- **Owning outcome:** **OUT-20** — the audit outcome, because the house-style publication and the independent cold read are where an ambiguous term is caught by a reader who has only the package.
- **Named owner:** **SUB-17 (NEU-1008)**, which owns the cold read that tests the collision from a reader's position rather than an author's.
- **Escalation route:** **`NEU-896`** at convergence, where a term colliding **across packages** is reconciled. An in-repository collision needs no escalation: it resolves through `docs/GLOSSARY.md` under the project's glossary discipline.
- **Mitigation:** Every use disambiguated at first occurrence; `subject` never used bare where the academic-subject sense is possible; a `docs/GLOSSARY.md` row in the same change for any new domain term. SUB-2 tested its own conformance mechanically and **recorded a false certification against itself** — an earlier revision of `G-S2-15` claimed the word appeared *"never bare"*, which was false in four places, all four reworded and the claim narrowed to what is checkable (`97_package-completeness-gate.md:90`). That correction is the mitigation working.
- **Mitigation status:** **Partially mitigated.** The discipline held for the three named terms: the independent cold read (`98_audit-record.md` §8.4) found no place where an unqualified use of `subject`, `session` or `context token` misled a reader with only the package. **Residual, named:** the glossary discipline is per-change and unenforced — no gate checks that a new domain term arrives with its row — so the mitigation depends on each author remembering it, and the package is now closed to further authoring. Owned by **`NEU-896`** for the cross-package sense and by the creator, as sole maintainer, for the repository glossary.

---

**SUB-17 register totals at revision 1:** four entries — `R5` (High), `R6` (High), `R7` (High),
`R15` (Low) — filling the four slots the charter owns to OUT-20 and SUB-14 reserved. Each carries a
severity, a mitigation, a named owner and an escalation route. **One is mitigated, two partially
mitigated, one open**; every non-mitigated status names its residual and that residual's owner.

**The register is now complete against the charter: 53 entries, and all fifteen charter § Risks rows
are authored** — SUB-1 ×3, SUB-3 ×2, SUB-5, SUB-6, SUB-7 ×2, SUB-9, SUB-11 and SUB-17 ×4, exactly the
allocation `92_risk-register.md:40`–`:42` names. No row is filled in on another author's behalf and
no id is renumbered.

**No amendment is routed to `NEU-895` by SUB-17 itself.** `R5` **reports** the amendment SUB-12
routed (`DR-C11-S12-2`) and confirms receipt as the named recipient; it does not mint a second record
of it. `R7`'s escalation route to `NEU-895` is the route that would carry a split-fidelity mismatch,
and **no mismatch was found**, so the route is named and unused.

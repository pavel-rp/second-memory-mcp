...
- **Mitigation status:** **Partially mitigated.** The discipline held for the three named terms: the independent cold read (`98_audit-record.md` §7) found no place where an unqualified use of `subject`, `session` or `context token` misled a reader with only the package. **Residual, named:** the glossary discipline is per-change and unenforced — no gate checks that a new domain term arrives with its row — so the mitigation depends on each author remembering it, and the package is now closed to further authoring. Owned by **`NEU-896`** for the cross-package sense and by the creator, as sole maintainer, for the repository glossary.

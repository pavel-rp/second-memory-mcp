# `DR-C11-S2-3` — Claim provenance is carried as a separate discriminator rather than re-derived or encoded into the key; and two shared id families are extended so fifteen authors never negotiate a number
...
5. **What a consumer may conclude from it.** Exactly this: **`user` means the token carried a `sub`
   ...
   principal is a natural person — that is `OI-S1-2` / `A-35`, still open. The discriminator records
...
### B. Two shared id families are extended for parallel-safe authoring
...
Both follow the existing `F-S<n>-<k>` / `OI-S<n>-<k>` / `CAP-S<n>-<k>` / `SPK-S<n>-<k>` shape, so no
new convention is invented — two families that were flat are brought into line with the six that
already are.
...
| 8 | **Defer both id families to SUB-14 at assembly.** | SUB-14 aggregates and authors nothing (charter assumptions 46, 47). Handing it unnumbered entries would require it to author ids, which is the rule it exists to preserve. |
...
4. **A reader of the discriminator is bounded by clause 5** and must not treat `user` as *human*.
   Every downstream sub-task that reads the kind inherits `A-35` with it.
5. **`R-S2-<k>` and `G-S2-<k>` enter the package's id vocabulary.** SUB-14 aggregates them like any
   other family; the risk register gains a stated reason why SUB-2 contributes zero `R<n>` rows, so
   its absence there reads as correct rather than as a routed gap against SUB-2.
6. **SUB-14 may supersede B entirely**, exactly as `DR-C11-S1-3` says of its own conventions. The
   cost is a mechanical rename plus a citation re-check.
7. **What becomes harder:** the package now has two id shapes in the risk register — `R<n>` for
   charter rows and `R-S2-<k>` for raised residuals. A reader must know which is which, and the
   register's own header is where that is stated.
...
5. **SUB-14 (`NEU-1007`) renumbers or supersedes either id family** at assembly. That is the expected

...
in one of the six enumerated copy classes, or in the one **named egress exception** of clause 5.
...
| `W-3` | Outbound network | `fetch(` / `axios` / `http.request` / `new OpenAI` / `createClient` across `src/` | **Three call sites; two carry learner content** | **Egress exception**, clause 5 |
...
> classes and the named egress exception.
>
...
> protocol response, platform backup, and this package's own activity. There is no eighth channel —
> a byte that is neither persisted, written to disk, sent over the network, held in memory, returned
> to the caller, backed up, nor captured by this package has not gone anywhere. `W-2` is empty by
...
**Clause 5 — the named egress exception, stated as a finding and not as a footnote.** `W-3` resolves
...
data whose destination is none of the six classes and is not one of clause 5's two named egress call
sites.** The falsification procedure is the enumeration itself, re-run: it is mechanical, takes four
greps, and requires no production access. It is stated so a later reader can attack the claim rather
than take it.

...
**Cumulative across SUB-1, SUB-2, SUB-15, SUB-16, SUB-4, SUB-8 and SUB-11: twenty-one spikes
designed, zero executed** — nine (`SPK-S1-1` … `SPK-S1-9`), four (`SPK-S15-1` … `SPK-S15-4`), three
(`SPK-S2-1` … `SPK-S2-3`), one (`SPK-S16-1`), two (`SPK-S4-1`, `SPK-S4-2`), one (`SPK-S8-1`) and
**one (`SPK-S11-1`)**. **The figure is re-derived by enumerating the register's own section
headings at this cutoff, not carried forward by adding one to SUB-8's note** — which is the method
`F-S4-6` prescribes after the same total was reported as twelve, sixteen, seventeen, eighteen and
twenty by five different authors on four different bases. SUB-5 filed zero spikes and is counted at
zero. No predecessor's text is edited; this register is append-only, and the reconciliation stays
routed to **SUB-14 (NEU-1007)** under OUT-20.

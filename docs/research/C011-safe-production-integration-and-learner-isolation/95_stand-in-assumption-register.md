...
- **Why a value is needed at all:** `R-S6-2` requires the sweeps to be batched, and `DR-C11-S7-2` clause 5 makes resumability the precondition of four of SUB-7's six disable paths. A batch needs a bound, and a runbook an implementer executes without asking a question cannot leave the bound blank.

...
- **Consequence:** **`R10` is already occupied.** The convention table directs **SUB-11 (NEU-1004)** to author the stale-tool-count risk as `R10`; under the register's own stated rule, and under the ids SUB-3 actually used, SUB-11's row is **`R11`**. A sub-task following the table rather than the rule would create a duplicate `R10` and leave `R11` empty — which is exactly the condition SUB-14's cross-register consistency check is defined to catch, and exactly the condition SUB-17 treats as blocking. SUB-11 is in flight at the time of writing, which is why this is filed rather than left for assembly.

...
| **C** | **An enumerated candidate set** — list items or table rows carrying source-native problem identifiers, the shape a retained problem list takes. | `grep -rncE '^(\s*[-*]\|\|)\s*.*\b(abc\|arc\|agc\|dp)[0-9]{2,4}_[a-z]\b' docs/research/C009-course-content-quality/` | **0 matches in each of the 7 files** (per-file counts all `0`). |

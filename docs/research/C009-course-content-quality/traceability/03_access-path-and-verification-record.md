...
**Change against SUB-1's baseline run, stated so the delta is visible rather than implied.** SUB-1 recorded scan D as *"0 matches — the package contains no fenced block at all, so no serialised response can be hiding in one."* **That is no longer true of the package**, and the claim it supported is no longer available: fenced blocks now exist, so **D can no longer prove anything by absence and must prove it by inspection instead.**

**The change is not this sub-task's alone, and the attribution is stated rather than quietly assumed:** the first fenced blocks were landed by **SUB-2** (its ten templates and its run-1 output); **SUB-3 adds a third file** (`dry-run/03_…`). A reader who assumed D still returned zero would draw a false conclusion about what the scan establishes — **what it now establishes is narrower, rests on inspection rather than on counting, and is stated narrowly.**

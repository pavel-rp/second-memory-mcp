...
| **`D-F3a`'s field constraint** | NEU-932, `../C005-dp-map-foundations/adjudication/01_selection-decision-ledger.md:27` and `../C005-dp-map-foundations/05_provenance-and-rights.md:54` | Two halves, cited to the line that carries each. The ledger row states the inherited constraint *"no field may hold verbatim external content"*; the rights document states it in full — *"no node field may hold verbatim external content; problem references, if any, are URLs and identifiers only"*. Used as the **admission** rationale for the two fields inside the bar (`01_…` §4), and as the **bar to argue against, not around**, for the four outside it. |

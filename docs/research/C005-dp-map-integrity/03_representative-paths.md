...
cl-1.formulate-sequence-partition-dp        PS-4
 ...
 -> cl-1.build-prefix-aggregate             PS-3
 -> cl-1.prefix-aggregate-recurrence        PS-2
...
cl-3.implement-steiner-tree-dp              PS-4
 ...
 -> cl-3.subset-cover-transition-correctness PS-3
 -> cl-3.submask-enumeration                PS-2
 -> cl-3.bitmask-state-encoding             PS-1   🚩 stage inversion at the next hop
...
🚩 **But this path is NOT consistent with NEU-940's stages.** `cl-3.bitmask-state-encoding`
is annotated **PS-1** while its prerequisite `cl-2.subset-sum-feasibility` is **PS-3**. A
learner following the stages would meet the dependent *two stages before* the thing it
requires. The **edge** is right; the **annotation** is wrong. This is **F-943-1** (§`04`)
and it is a defect in the annotation layer, not in the graph.
...
cl-4.implement-modular-matrix-power         PS-4
 ...
 -> cl-3.formulate-automaton-dp             PS-2   [cross-cluster: CL-4 -> CL-3]
 -> cl-1.linear-sequence-dp-2d              PS-3   🚩 inversion (PS-2 requires PS-3)
 -> cl-1.linear-sequence-dp-1d              PS-2
 -> cl-1.linear-dp-pattern-catalogue        PS-1
 -> cl-1.root.state-transition-base-case-formulation   [ROOT — frozen]
```
...
cluster, not only through CL-1. Structurally clean; carries one **F-943-1** stage
inversion at the CL-3→CL-1 hop.
...
**This is the cleanest path in the set** — and notably the only one of the five walked in
full that is *both* structurally and stage-consistently clean. It descends from an online
linear-time SMAWK implementation to *optimal substructure* in four hops with no jump.
...
**Split verdict. Stated separately because they differ:**

| Property | Result |
| -------- | ------ |
| Every path reaches the sanctioned floor | ✅ **5/5** |
| Every path free of unexplained jumps | ✅ **5/5** |
| Every path consistent with NEU-940's `progression_stage` | ❌ **3/5** — CL-3 and CL-4 carry inversions |
| Every path consistent with NEU-940's `difficulty_dimensions` key-set | ✅ 179/179 share one key-set |
| Declared `prerequisite_depth` matches the walked depth | ❌ **153/179** — 26 under-report |

OUT-6 requires each path be *"consistent with the NEU-940 progression stages and
difficulty dimensions."* **Two of the five walked paths are not**, and the depth
annotation disagrees with the graph on 26 nodes. Both defects have **one** root cause and
are recorded as **F-943-1** (§`04`).

This is reported as a **flagged finding against the annotation layer, not as a path
failure**: the paths themselves are structurally sound — every hop exists and every
terminal is sanctioned. What fails is NEU-940's labelling of nodes those paths cross.
Smoothing this — by quietly re-staging the six nodes — would have edited node files this

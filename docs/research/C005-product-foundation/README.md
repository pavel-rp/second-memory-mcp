...
| 4 | **`measurement-contracts/`** | The measurement authority: frozen contracts MC-1…11 (v1.0), the requirement-decision mapping gate (GATE-STATE=PASS), per-signal feasibility (FEAS-*), proxy-replacement contracts (PRX-1…8, the INC-4 artifact), and the operational-log privacy gate (PLA-1…3, all PASS). Binds LINK-2. | NEU-901 |

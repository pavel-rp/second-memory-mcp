...
- **NEU-899 traceability structure** (built on the NEU-898 model) lives in `traceability/` — stable per-element trace records with bidirectional evidence↔behavior/metric/decision-rule/rejected-alternative links, an explicit completeness lattice (SETTLED / PROVISIONAL / INCOMPLETE / UNRESOLVED), `INC-*` markers for missing downstream artifacts, `LINK-*` slots for later sub-tasks to bind, and forward/reverse orphan audits. Start at `traceability/README.md`.

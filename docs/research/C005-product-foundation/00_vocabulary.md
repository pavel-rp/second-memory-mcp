...
**Definition:** per-signal feasibility checked against real code; verdict vocab COMPUTABLE / COMPUTABLE-UNVALIDATED / UNCOMPUTED / UNAVAILABLE. **This nine-signal inventory is the canonical, complete answer to "which learner metrics are actually collectible today"** — go here (not to this vocabulary) for the exhaustive per-signal list. **Defining file:** `measurement-contracts/02_feasibility-and-telemetry-inventory.md`; the underlying code findings are RQ4's F4.1–F4.5 (`questions/RQ4.md`). Headline verdicts (illustrative, not the full nine): per-attempt data is persisted (F4.1, COMPUTABLE); `time_spent_ms` COMPUTABLE-UNVALIDATED (reliability uncharacterized, G4.1/MC-8); `averageQuality` UNCOMPUTED (hardcoded `0`, F4.3/MC-7); per-DP-pattern mastery UNAVAILABLE (no schema field, G4.2/MC-6). Net: almost nothing is validated-and-collectible today — most desired signals are COLLECTION-GAP pending INC-2.

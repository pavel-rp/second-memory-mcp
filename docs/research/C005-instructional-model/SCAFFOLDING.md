...
**NEU-922 integrative mastery model (binds `LINK-I2`):** the single cross-mechanism operational mastery model consolidating the ten `DR-Mxx` per-mechanism mastery signals lives in `mastery-model/00_operational-mastery-model.md`. It fixes the unified mastery-signal spine, the staged promotion/progression decision rules, and — the load-bearing deliverable — a threshold table that binds `LINK-I2`: every DR-deferred value (`MM-T1…MM-T15`) receives a **provisional value, an explicit epistemic uncertainty band, an evidence class, and the signal that would revise it**. It **resolves no conflict** (C1/C4 stay `unresolved`·non-downgradable — reconciliation is NEU-923), keeps `INC-I1` (DP transfer) provisional, claims **no external validity** (population validity UNEARNED until dogfooding/production), and contradicts no `DR-Mxx` invariant (DR-M10 durability gate, DR-M04 inter-session gate, DR-M08 deterministic mapper). Ledger rows in `adjudication/01_…` §C-MASTERY (see §SELF-CHECK-922); register binding note in `traceability/01_…` (NEU-922 block). **Nothing is `settled`.**

**NEU-923 reconciliation conflict register (discharges `INC-I3`):** the register reconciling the target model against the coded Second Memory learning model lives in `reconciliation/00_conflict-register.md`. For each of the **ten mechanisms** (M01–M10) plus the **fifteen mastery thresholds** (MM-T1…MM-T15) and all six conflicts (C1–C6) it records an **alignment / gap / conflict** verdict against the **current coded behavior**, read directly from `src/domain/algorithms/{sr-calculator,classify-chunk,resolve-stale-prerequisites}.ts`, `src/domain/config/algorithm-defaults.ts`, `src/orchestration/teaching-workflows.ts`, and `src/shared/instructions.ts` as a **class-2 compatibility fact, never accepted pedagogy**. It verifies the audit's three flagged behaviors against live source — **correcting** the prerequisite-gate characterization (live gate is retrievability-reteach at 0.5, not `repetitions>0`) and the leech "dead code" note (`leechFailureThreshold=6` is a live lifetime floor), and **confirming** the lapse full-reset (C3) and LLM over-validation (C4) conflicts. Every confirmed conflict/gap states the required target behavior and — for every learning-critical divergence — the non-prose enforceable control (owned by the relevant `DR-Mxx`; the register invents none). It **modifies no source file**, **resolves no conflict** (C1/C4 stay `unresolved`·non-downgradable; implied code changes route to a later implementation charter), invents no MM-T value, and keeps `INC-I1` (DP transfer) provisional. Ledger rows in `adjudication/01_…` §RECON (see §SELF-CHECK-923); C1/C4 §CONFLICTS cells bound to the register. **Nothing is `settled`.**

**NEU-924 targeted in-charter experiments (OUT-6):** the experiments package lives in `experiments/` — `00_experiment-inventory-and-ranking.md` (materiality inventory over the DR residuals / MM-T bands / reconciliation verdicts, charter-risk ranking, smallest-sufficient-vehicle selection per NEU-887's toolkit, six-item non-deferrable-vehicle cap application), the six executed experiment records `01_…`–`06_…` (2× AI review under the independence protocol — M03/C2, M06/C6, both unanimous `supports`; 4× automated-eval with versioned frozen cases, explicit oracles, recorded ENV + CCR evidence, ≥2 isolated repeats — M04/C2, M08/C4, M09/C3, M10/C1), and `07_deferral-register.md` (every creator-dogfooding-vehicle experiment deferred — creator unavailable, Assumption #10 — plus cap-overflow and untestable items, each shipping provisional with an explicit revision trigger; includes the "ships without dogfooding evidence" list). Results are class-4/5 findings (`F-EXP-01…06`) registered in `traceability/01_…` §EXP and attached to their decisions via the ledger's §EXP section (see §SELF-CHECK-924). **Attaches evidence only — no experiment settles a decision, resolves a conflict (C1–C6 stay `unresolved`), calibrates an MM-T value, or claims external validity; no source file modified; no settled point over-tested.**

**NEU-925 assembled decision package (OUT-7, final sub-task — drives the ledger):** the standalone, prompt-ready package that unifies NEU-915…NEU-924 into one artifact lives in `package/` — `README.md` (the package map + reading order + final tally + standing caveats), `00_per-mechanism-index.md` (**the load-bearing deliverable** — one block per mechanism M01–M10 recovering, in one hop, evidence+class · observable behavior · mastery signal · reconciliation verdict · uncertainty · rejected alternative · experiment evidence, plus binding-vs-open status), `02_ships-without-evidence.md` (the decisions shipping without dogfooding evidence D-1…D-6 and without cap-covered experiment evidence O-1/O-2, symmetric, each provisional with a revision trigger; untestable residue U-1), and `03_completeness-gate-and-dry-run.md` (the 4/4 package-completeness gate + the written cold-context dry-run handoff over M08/M10). NEU-925 **drives the whole ledger** (`adjudication/01_…` §DRIVE / §SELF-CHECK-925): every element re-tested against the template's flip criteria and labeled `settled`/`provisional`/`unresolved`. **Final tally: 0 settled · 15 provisional · 14 unresolved.** It **authors no new mechanism decision, threshold, verdict, or experiment**; flips **nothing** to `settled` (class-7 absent, `INC-I1` open); C1–C4 stay `unresolved`·non-downgradable; modifies no source file. **Nothing is `settled`.**

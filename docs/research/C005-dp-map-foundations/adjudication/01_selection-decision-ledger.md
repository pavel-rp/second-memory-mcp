...
## 6. Challenges filed by downstream charters

**Appended by union, never by replacement.** This section is added by a downstream charter under the `A1`–`A5` status discipline: a challenge is **filed** here, as a new row, and **§1–§5 above are unchanged** — no existing decision, marker, conflict or self-check row is edited, renamed, reordered or removed by it. A challenge does **not** alter the status of the decision it challenges until this ledger records a disposition for it. `D-F5` therefore remains **settled** in §2, and `CH-F5-1` is open beside it.

| Id | Challenges | Filed by | Scope of the challenge | Status | Owner | Revision trigger | Supporting record |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **CH-F5-1** | **`D-F5`** (and, through it, the `D-F3a` field constraint at `:27` — *"no field may hold verbatim external content"*, stated in full at `../05_provenance-and-rights.md:54` as *"no node field may hold verbatim external content; problem references, if any, are URLs and identifiers only"*) | **NEU-957 (SUB-1) of charter C009**, *Specify the course content, problem sourcing and assessment package* (umbrella **NEU-890**) | **Every problem-reference field beyond the stable id and the canonical URL** — namely **`title`**, numeric **`constraints`**, **difficulty signal**, and **curriculum placement**. The stable id and the canonical URL are **not** challenged: they are admitted on `D-F3a`'s own recorded rationale, which names ids and URLs in its own words. | **unresolved / open** | **NEU-932** — `D-F5`'s own owner — **or the creator by default.** The filer does not own the resolution: **a producing task may not promote its own artifact** (`A1`–`A5`), so NEU-957 argues the position and files it here rather than deciding it. | **This ledger records a disposition for `CH-F5-1`.** That is the only route. It does not resolve by a downstream sub-task deciding it needs the fields, by an execution result from a citation pass, or by charter assumption 19 being restated as fact. | `../../C009-course-content-quality/01_provenance-and-rights.md` §4 (all six candidate fields with a per-field rights rationale) and §4.1 (the interim position) · `../../C009-course-content-quality/decision-records/DR-C09-01_permitted-field-set.md` (the decision record, which sets no status of its own) |

**Why it was filed rather than decided locally.** `D-F5` §5 requires that a proposal to store anything beyond ids and canonical URLs **file a ledger challenge and never proceed on local judgment**. C009's reference-only product design assumes a wider record, and charter assumption 19 holds that `title` and numeric `constraints` are *provisionally* facts about a source rather than its expression — but assumption 19 is **`[unconfirmed]`**, and the four contested fields also engage `../05_provenance-and-rights.md` §2's selection-and-curation bright line: a stored table of titles, bounds or ratings across many problems reproduces a source's selection and evaluative curation independently of any no-text rule. Neither ground is C009's to settle by itself.

**The interim position, stated explicitly, binding until this challenge resolves.** **A problem reference stores `stable id` and `canonical URL` — and nothing else.** Every C009 sub-task is bound to that set; a sub-task needing a wider one cites `CH-F5-1` by id, records its position for both dispositions, and carries the unresolved field set as a cap with a named owner, rather than widening locally. **Charter assumption 19 remains `[unconfirmed]`** and is relied on by nothing. Recording a narrow interim set is not a finding that the wider set is impermissible — it is a refusal to act on an undecided question in the permissive direction.

**Blast radius if it resolves either way.** Small and deliberately so. C009's source dispositions, its per-source access-permission record, its no-text rule and its enumerating-response retention rule are all independent of the field set. If `CH-F5-1` succeeds, the four fields become admissible with no other change; if it fails, nothing already written has to be withdrawn.

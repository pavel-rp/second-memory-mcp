...
export function validateContentSize(content: string, type: ContentType): ContentValidationResult {
	...
	const maxSize = type === "chunk" ? VALIDATION_CONSTANTS.MAX_CONTENT_SIZE : VALIDATION_CONSTANTS.MAX_SUMMARY_SIZE;
 ...
 */
export function sanitizeContent(content: string, options: { preserveBasicMarkdown?: boolean; preserveNewlines?: boolean } = {}): string {
	if (!content || typeof content !== "string") {
		return "";
	}
 ...
 */
export function validateContent(content: string, type: ContentType, options: { sanitize?: boolean; preserveBasicMarkdown?: boolean } = {}): ContentValidationResult {
	// First validate size
 ...
 */
export function validateContentBatch(contentItems: Array<{ content: string; type: ContentType; id?: string }>, options: { sanitize?: boolean; preserveBasicMarkdown?: boolean } = {}): Array<ContentValidationResult & { id?: string }> {

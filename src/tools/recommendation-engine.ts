...
export class RecommendationEngine {
	...
	generateRecommendations(input: RecommendationInput): RecommendationOutput {
		...
		const conversationGuidance = processedInput.mode === "guided"
	...
	private applyIntelligentDefaults(input: RecommendationInput): RecommendationInput {
		...
		if (!input.subjectPreference && input.userHistory?.patterns.subjectPreferences) {
			...
			const topSubject = Object.entries(preferences)
				.sort(([,a], [,b]) => b - a)[0]?.[0];
	...
	private filterAndPrioritizeCandidates(
		...
		if (constraints?.subjectFilter) {
			filtered = filtered.filter(item =>
				item.subject === constraints.subjectFilter
			);
		}
		...
		if (constraints?.excludeIds?.length) {
			filtered = filtered.filter(item =>
				!constraints.excludeIds!.includes(item.id)
			);
		}
	...
	private composeBalancedSession(
		...
		const overdueItems = candidates.filter(item =>
			new Date(item.nextReviewDate) <= new Date()
		);
		const reviewItems = candidates.filter(item =>
	 ...
	 */
	private interleaveRecommendations(recommendations: LearningRecommendation[]): LearningRecommendation[] {
	...
	private generateConversationGuidance(
		...
		if (recommendations.length === 0) {
			return {
				nextAction: "No items are due for review right now. Consider studying new content or taking a break.",
		...
		if (recommendations.length > 1) {
			nextAction += ` Then continue with ${recommendations.length - 1} more items (~${totalTime} min total).`;
		}
	...
	private generateAlternatives(
		...
		const alternatives = candidates
			...
			.map((item, index) => this.createRecommendation(
				item,
				index + 1,
				"alternative option"
			));

...
export class PromptPack {
  ...
  private getWorkflowGuidancePrompt(): string {
    return [
      ...
      '4) Record: submit_answer({ question: "...", response: "...", passed: true/false, feedback: "...", time_spent_ms: <elapsed_ms> }). On "retry", ask the learner to try again and resubmit until "recorded". After "recorded", check next: if next.status is "teach", go to step 3 for that chunk (already in_progress — do not call create_session_chunk). Loop to step 2 only when next.status is "complete" or no chunk is currently in_progress.',

...
class PromptPack {
  ...
  private getScaffoldingPrompt(context: PromptContext): string {
    ...
    return [
      ...
      '2) PROBE PRIOR KNOWLEDGE: ask the learner targeted questions about key prerequisites. Absence from the database does not mean the learner lacks knowledge — probe before assuming a gap.',
      '',
      'Produce:',
  ...
  private getReviewPrompt(context: PromptContext): string {
    ...
    const basePlan = [
      ...
      '5) Evaluate quality using the rubric below',
    ];
  ...
  private getWorkflowGuidancePrompt(): string {
    return [
      ...
      '',
      '## Probe-First Scaffolding (Proactive Flow)',
      '',
      ...
      '3) Probe: for prerequisites not found in the database, ask the learner targeted questions — e.g., "Can you explain path compression in Union-Find?"',
      ...
      '1) Search existing content and probe the learner (see Probe-First Scaffolding above)',
  ...
  private getLearningSessionPrompt(context: PromptContext): string {
    ...
    return [
      ...
      '2. Probe: if not found, ask the learner targeted questions about the topic before assuming a gap — absence from the DB does not mean the learner lacks knowledge',
  ...
  private formatQualityRubric(): string[] {
    return [
      '## Grading Rubric (rubric-anchored — you do NOT supply a raw quality number)',
      '',
      "You do not emit a 0–5 quality. Instead, judge the learner's answer against the",
      'rubric criteria and pass a `grading` payload to submit_answer / revise_grade. The',
      'server derives the 0–5 quality deterministically from it. Rubric criteria (weights):',
      '',
      '- core_correctness (2): the central claim or conclusion is correct for the question asked.',
      '- completeness (1): the substantive parts the question asks for are covered, with no material gap.',
      '- reasoning_validity (1): any reasoning or steps the question calls for are logically valid.',
      '  If the question asks for no reasoning (e.g. pure recall), a correct answer satisfies it.',
      '- precision (1): any specificity the question calls for (exact values, terminology, units,',
      '  complexity) is accurate. If the question asks for none, a correct answer satisfies it.',
      '',
      'Judge only against what the question actually asks. Never penalize the omission of',
      'something the question did not ask for, and never invent evidence for a criterion.',
      '',
      'For each criterion set a boolean, and — for every criterion you mark true — supply a',
      "verbatim `justifying_spans` entry quoting the learner's OWN words that evidence it.",
      '',
      'Rules:',
      '- A criterion marked true WITHOUT a verbatim justifying span is uncredited (the',
      '  derived quality fails closed). Never invent, paraphrase, or supply flattering spans.',
      '- Base every boolean on demonstrated understanding in the answer, not on self-report,',
      "  and not on a learner's rebuttal. A bare rebuttal changes nothing — regrade only by",
      '  submitting a NEW rubric payload via revise_grade.',
      '- The derived quality (correctness 2 + completeness 1 + reasoning 1 + precision 1)',
      '  preserves full 0–5 granularity; a pass is quality >= 3. You cannot certify a pass',
      '  without evidence.',
      '',
      'grading payload shape:',
      '  { criteria: { core_correctness, completeness, reasoning_validity,',
      '                precision },  // all four booleans required',
      '    justifying_spans: { core_correctness?: "...", ... } }',
    ];
  }

...
class PromptPack {
  ...
  private formatQualityRubric(): string[] {
    return [
      ...
      '',
      '- correct_recurrence (2): the recurrence / transition relation is correct.',
      '- correct_base_case (1): the base case(s) are correct.',
      '- correct_iteration_order (1): the fill/iteration order (or memoization order) is correct.',
      '- complexity_stated (1): the time/space complexity is stated correctly.',
      '',
      ...
      '- The derived quality (recurrence 2 + base 1 + order 1 + complexity 1) preserves full',
      '  0–5 granularity; a pass is quality >= 3. You cannot certify a pass without evidence.',
      '',
      'grading payload shape:',
      '  { criteria: { correct_recurrence, correct_base_case, correct_iteration_order,',
      '                complexity_stated },  // all four booleans required',
      '    justifying_spans: { correct_recurrence?: "...", ... } }',
    ];
  }

...
class PromptPack {
  ...
  private getWorkflowGuidancePrompt(): string {
    return [
      ...
      '4) Record: submit_answer({ question: "...", response: "...", passed: true, feedback: "...", time_spent_ms: 5000 }) — set passed and time_spent_ms to the actual result values. On "retry", ask the learner to try again and resubmit until "recorded". After "recorded", check next: if next.status is "teach", go to step 3 for that chunk (already in_progress — do not call create_session_chunk). If next.status is "blocked" or "error", surface the message to the learner and stop. Loop to step 2 only when next.status is "complete" or no chunk is currently in_progress.',
      ...
      '   - add_note({ note_type: "confusion", ... }) to record the gap (interim: use "confusion" note type)',

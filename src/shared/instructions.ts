...
4. On data.action "recorded", if roadblock_forecast is present, follow-up questions are required before progression — prepare accordingly. Call teach_next to get the next action: "teach" → present instruction, repeat from step 2. "roadblock" → follow-up questions are required before the server allows progression; follow roadblock_detail.instruction to scaffold the learner. "complete" → go to step 5. "blocked"/"error" → stop.
...
4. Call submit_answer with prompt_text, chunk_ids, response, pass/fail judgment, feedback, and time_spent_ms. If data.action says "retry", follow retry_guidance if present: use the mode-specific pivot to scaffold the learner, and re-call submit_answer with session_question_id from the response until it returns "recorded". After "recorded", if roadblock_forecast is present, follow-up questions are required before progression — prepare accordingly. Call teach_next to advance: if data.action is "teach", go to step 3 for that chunk (it is already in_progress — do not call create_session_chunk). If data.action is "roadblock", follow-up questions are required; follow roadblock_detail.instruction. If data.action is "blocked" or "error", surface the message to the learner and stop. Only loop back to step 2 when data.action is "complete" or no chunk is currently in_progress.

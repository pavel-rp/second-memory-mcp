...
export class TopicCreationService {
	...
	async updateTopic(topicId: string, updates: { title?: string }): Promise<{
		...
		try {
			...
			const currentTopic = db.select().from(learningTopics).where(eq(learningTopics.id, topicId)).get();
			if (!currentTopic) {
				return {
					success: false,
					error: {
			...
			// Update topic
			const res = db.update(learningTopics).set(updateData).where(eq(learningTopics.id, topicId)).run();
			...
			const updatedTopic = db.select().from(learningTopics).where(eq(learningTopics.id, topicId)).get();
			return {
				success: true,
	 ...
	 */
	async updateTopicSummary(topicId: string, summary: string): Promise<{
		success: boolean;
		topic?: LearningTopicRow;
		error?: {
			type: "validation" | "not_found" | "database";
		...
		try {
			...
			const res = db.update(learningTopics).set({
				summary,
				summaryVersion: newVersion,
				summaryUpdatedAt: now,
				updatedAt: now,
			}).where(eq(learningTopics.id, topicId)).run();

...
export const CalculateNextReviewInputShape = {
  quality: z
    ...
    .min(0)
    .max(5)
    .describe("Quality score from the latest review (0-5)"),
  repetitions: z
    .number()
    .int()
    .min(0)
    .describe("Number of successful repetitions completed"),
  ease_factor: z
    .number()
    .min(1.3)
    .describe("Current ease factor for the learning item"),
  interval: z
    .number()
    .int()
    .min(0)
    .describe("Current review interval in days"),
} as const;

export const CalculateNextReviewInputSchema = z.object(
  CalculateNextReviewInputShape,
);
export type CalculateNextReviewInput = z.infer<
  typeof CalculateNextReviewInputSchema
>;

export const CalculatePriorityScoreInputShape = {
  next_review_date: z
    .string()
    .describe("Upcoming review date in ISO format (YYYY-MM-DD)"),
  ease_factor: z
    .number()
    .min(1.3)
    .describe("Ease factor used for scheduling"),
  repetitions: z
    .number()
    .int()
    .min(0)
    .describe("Total successful repetitions completed"),
  difficulty: z
    .number()
    .int()
    .min(1)
...
export type CalculatePriorityScoreInput = z.infer<
  typeof CalculatePriorityScoreInputSchema
>;
...
export const RankCandidateShape = {
  ...
  next_review_date: z
    .string()
    .describe("Scheduled review date in ISO format (YYYY-MM-DD)"),
  ease_factor: z
    .number()
    .min(1.3)
    .describe("Current ease factor for the learning item"),
  repetitions: z
    ...
    .int()
    .min(0)
    .describe("Total number of successful repetitions"),
  difficulty: z
    .number()
    .int()
    .min(1)
    .max(10)
    .describe("Difficulty rating (1-10)"),
  tags: z
    .array(z.string())
    .optional()
    .describe("Optional tag metadata for the item"),
} as const;
...
export type RecordReviewResultInput = z.infer<
  typeof RecordReviewResultInputSchema
>;
